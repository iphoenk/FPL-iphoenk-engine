...
def _bounded_ins_states(cur,outids,need,bp,budget,beam=40):
    ...
    for pos in slots:
        ...
        for chosen,cost,clubs,score in states:
            ...
            for p in bp[pos]:
                ...
                risk_score=p.objective-.12*p.uncertainty
                nxt.append((chosen+(p,),cost+p.cost,cc,score+risk_score))
...
def _candidate_states(cur,outids,need,bp,budget,k,beam_size):
    ...
    for comboset in pools:
        ...
        for s in states:
            for c in comboset:
                t=s+c
                if len({p.element for p in t})!=len(t): continue
                nxt.append(t)
        states=nxt
    # Even for 1-2 changes, evaluate only the strongest legal shortlist.
    keep=[p for p in cur if p.element not in outids]; keep_cost=sum(p.cost for p in keep); clubs=Counter(p.team_id for p in keep)
    legal=[]
    for chosen in states:
        ...
        if not ok: continue
        quick=sum((p.objective-.12*p.uncertainty) for p in chosen)
        legal.append((quick,chosen))
    legal.sort(key=lambda x:x[0],reverse=True)
    cap=24 if k==1 else 48
    return [x[1] for x in legal[:cap]]

def audit_packages(predictions,universe,locked,max_replacements=4,budget=BUDGET_TENTHS,per_position_frontier=8,top_per_size=10,beam_size=40):
    ...
    baseline_unc=sum(p.uncertainty for p in cur)
    for k in range(1,max_replacements+1):
        ...
        for outs in combinations(cur,k):
            ...
            states=_candidate_states(cur,outids,need,bp,budget,k,beam_size)
            for chosen in states:
                ...
                target_unc=sum(p.uncertainty for p in target); risk_delta=target_unc-baseline_unc
                risk_penalty=max(0.0,risk_delta)*0.35 + max(0,k-1)*0.20
                adj_xi=dxi-risk_penalty; adj_util=du-risk_penalty
    ...
    return {'schema_version':443,'engine':'v4.4.2-wc-package-audit-fast-risk','wildcard_active':bool(locked.get('wildcard_active')),'baseline':cm|{'itb':budget-basecost},'screened_players':len(cands),'frontier_players':len(fr),'max_replacements':max_replacements,'best_by_replacement_count':best,'packages':results,'overall_verdict':verdict,'recommended_package':overall,'guardrails':{'max_per_club':MAX_PER_CLUB,'budget_tenths':budget,'position_counts':POSITION_COUNTS,'larger_packages_require_higher_gain':True,'ranking_metric':'risk-adjusted best-XI plus bench-adjusted 5GW utility','search':'shortlisted k<=2, bounded beam k=3-4','frontier_per_position':per_position_frontier,'beam_size':beam_size,'risk_penalty_enabled':True}}
def run():
