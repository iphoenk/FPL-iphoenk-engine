...
def test_s02_explicit_planning_gw_identity_beats_previous_gw_public_fallback(tmp_path: Path):
    stale_ids = [
        572, 109,
        115, 8, 31, 391, 279,
        426, 565, 40, 15, 68,
    ...
    stale_picks = [
        {"element_id": element_id, "squad_position": index}
        for index, element_id in enumerate(stale_ids, start=1)
    ]
    _write(
        tmp_path / "data/v6/personal/submitted_picks.json",
        {
    ...
    _write(
        tmp_path / "data/v6/personal/current_team.json",
        {
            "entry_id": 3462711,
            "gw": 5,
    ...
    resolved = runner._personal_evidence_resolution(
        tmp_path,
        {"confirmed_current_squad_state": confirmed},
        planning_gw=6,
    )
...
def test_s02_previous_gw_public_identity_remains_stale_without_current_confirmation(tmp_path: Path):
    stale_ids = [
        572, 109,
        115, 8, 31, 391, 279,
        426, 565, 40, 15, 68,
