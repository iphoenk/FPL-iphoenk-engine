...
def capture(
    ...
    *,
    ...
    # First genuine pre-deadline capture for a GW is frozen. Later runtime cycles
    # may observe it but never reconstruct/overwrite it after the fact.
    ...
    for row in lineup.get("starters") or lineup.get("starting_xi") or []:
        ...
        if element is not None: starters.append({"element":element,"position":row.get("position") if isinstance(row,dict) else None})
    owned = []
    for row in team.get("squad") or []:
        element = _element(row)
        if element is not None: owned.append({"element":element,"position":row.get("position") if isinstance(row,dict) else None})
    ...
    for row in comparator.get("top_comparisons") or []:
        ...
        if out_id is None or in_id is None: continue
...
def decision_regret(snapshot: dict[str, Any] | None, actual: dict[int, dict[str, Any]]) -> dict[str, Any]:
    ...
    if not isinstance(snapshot,dict): return missing
    ...
    if len(xi)==11 and all(x in actual for x in xi):
        selected=sum(float(actual[x].get("points") or 0.0) for x in xi); missing["xi_regret"]={"status":"SETTLED_SELECTED_XI_ONLY","sample_size":1,"value":None,"selected_xi_actual_points":round(selected,4),"note":"best legal XI regret requires reliable owned positional evidence at settlement"}
    gross_rows=[]; exact=[]
    for row in (snapshot.get("comparator") or {}).get("comparisons") or []:
        ...
        if out_id not in actual or in_id not in actual: continue
        ...
        if net is not None: exact.append(net)
    if gross_rows: missing["transfer_comparator_realized_net_gain"]={"status":"SETTLED" if exact else "PARTIAL_GROSS_ONLY","sample_size":len(exact),"value":round(sum(exact)/len(exact),4) if exact else None,"gross_pair_count":len(gross_rows),"comparisons":gross_rows}
    return missing
