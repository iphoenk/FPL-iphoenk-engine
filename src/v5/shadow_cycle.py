...
def run(v3_latest_path: str, v3_lineup_path: str, output_dir: str, team_id: int) -> dict[str, Any]:
    ...
    service_performance = v5.get("service_performance") if isinstance(v5.get("service_performance"), dict) else {}
    core_wall_ms = float(service_performance.get("orchestrator_wall_ms") or 0.0)
    full_beta_wall_ms = float(service_performance.get("full_beta_end_to_end_ms") or 0.0)
    core_hard_ms = float(budgets.get("decision_pipeline_hard_limit_seconds") or 0.0) * 1000.0
    full_beta_hard_ms = float(budgets.get("full_beta_end_to_end_hard_limit_seconds") or 0.0) * 1000.0
    core_runtime_pass = core_wall_ms > 0.0 and core_hard_ms > 0.0 and core_wall_ms <= core_hard_ms
    full_beta_runtime_pass = (
        full_beta_wall_ms > 0.0
        and full_beta_hard_ms > 0.0
        and full_beta_wall_ms <= full_beta_hard_ms
    )
    ...
    invariants = {
        ...
        "core_runtime_under_hard_limit": core_runtime_pass,
        "full_beta_end_to_end_under_one_second": full_beta_runtime_pass,
    }
    ...
    result = {
        ...
        "acceptance_context": {
            ...
            "runtime_requirement": {
                "core_hard_ms": core_hard_ms,
                "full_beta_hard_ms": full_beta_hard_ms,
    ...
    print(json.dumps({"cycle_id": cycle_id, "cycle_pass": cycle_pass, "post_validation_status": post_status, "parity_pass": parity.get("pass"), "core_wall_ms": core_wall_ms, "full_beta_end_to_end_ms": full_beta_wall_ms, "release_fingerprint": release["fingerprint"], "output": str(cycle_path)}, ensure_ascii=False))
    return result
