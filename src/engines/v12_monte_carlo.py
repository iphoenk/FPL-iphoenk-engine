...
def _calibrated_poisson_base_mean(
    ...
    """Calibrate a Poisson-lognormal mean to the Stage-2 zero-goal marginal.

    The shared match and team factors imply a lognormal marginal attack factor
    with variance parameter match_sigma^2 + team_sigma^2.  Gauss-Hermite
    quadrature solves E[exp(-lambda * factor)] = target_zero deterministically,
    so calibration is invariant to Monte Carlo chunk size and is not repeated
    over every sampled path.
    """
