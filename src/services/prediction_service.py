...
def run() -> dict:
    ...
    if picks and live:
        by_id = {p["id"]: p for p in bootstrap["elements"]}; live_by_id = {p["id"]: p for p in live.get("elements", [])}; detail = []; gross = 0
        for pick in picks.get("picks", []):
            ...
            detail.append({"element": pick["element"], "name": player.get("web_name"), "pick_position": pick.get("position"), "multiplier": pick.get("multiplier"), **stats})
    ...
    latest = {"schema_version": 481, "engine_version": "4.8.1-independent-services", "generated_at": generated, "mode": raw["mode"], "checkpoint_context": raw["checkpoint_context"], "team_id": raw["team_id"], "phase": phase, "endpoint_health": raw["endpoint_health"], "squad_authority": raw["squad_authority"], "advanced_stats_sync": enrichment["advanced_stats_sync"], "prediction_summary": {"model": predictions["model_version"], "players": len(predictions["players"]), "top_5gw": predictions["players"][:10]}, "team_summary": {"itb": itb, "market_value": sum(x["now_cost"] for x in ledger), "sell_value": sum(x["sell_cost"] for x in ledger if x["sell_cost"] is not None)}, "live_summary": {"status": live_payload["status"], "gross_points": live_payload.get("gross_points"), "net_points": live_payload.get("net_points")}, "lineage": {"snapshot_sha256": snapshot_sha, "enrichment_sha256": enrichment_sha}, "files": {"team": "data/team.json", "universe": "data/universe.json", "predictions": "data/predictions_v4.json"}, "performance": {"prediction_ms": round((perf_counter()-started)*1000, 2)}, "meta": {"direct_fpl_api_authority": False, "raw_snapshot_is_official_api_authority": True, "fail_closed": True, "prediction_point_in_time": True, "simulation_never_authorizes_action": True, "service_contract_compatible": True, "service_boundaries_registry_driven": True}}
