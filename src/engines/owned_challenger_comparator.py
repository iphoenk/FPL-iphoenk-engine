...
def _package_map(payload: dict[str, Any]) -> dict[tuple[int, int], dict[str, Any]]:
    ...
    for package in payload.get("packages") or []:
        if package.get("legal") is not True or (package.get("score") or {}).get("valid") is not True:
            continue
        outs = package.get("outs") or []; ins = package.get("ins") or []
        if len(outs) != 1 or len(ins) != 1:
            continue
        key = (_i(outs[0].get("element")), _i(ins[0].get("element")))
        robust = _f((package.get("score") or {}).get("robust_score"))
        out[key] = {"package_id": package.get("id"), "robust_gain_vs_hold": round(robust - hold, 3), "resulting_itb": (package.get("affordability") or {}).get("resulting_itb"), "legal": True}
    return out
...
def _match_stats() -> dict[int, dict[str, float]]:
    ...
    for row in payload.get("rows") or []:
        ...
        agg["returns"] += _f(row.get("goals")) + _f(row.get("assists")); agg["xgi"] += _f(row.get("xg")) + _f(row.get("xa"))
        agg["shots"] += _f(row.get("total_shots")); agg["box_touches"] += _f(row.get("touches_opposition_box")); agg["chances_created"] += _f(row.get("chances_created"))
    return out
...
def _critical_evidence(out: dict[str, Any], incoming: dict[str, Any], affordable: bool, club_legal: bool, external: dict[str, Any]) -> dict[str, str]:
    out_proj = out.get("projection") or {}; tactical = incoming.get("tactical_matchup") or {}
    load = read_json(DATA / "recent_competitive_load.json", {}); load_rows = load.get("players") or {}
...
def _actionability(row: dict[str, Any]) -> dict[str, Any]:
    cfg = load_policy().get("decision") or {}; state = str(row.get("state") or "HOLD")
    h3 = _f((((row.get("horizons") or {}).get("3") or {}).get("projected_edge")))
    h5 = _f((((row.get("horizons") or {}).get("5") or {}).get("projected_edge")))
    finance = row.get("finance") or {}; legality = row.get("legality") or {}; tactical = row.get("tactical_matchup") or {}
    start = _f((row.get("start_probability") or {}).get("challenger")); missing = list(row.get("missing_critical_evidence") or [])
    timing = str(row.get("price_urgency") or "MODEL_CONTEXT_ONLY")
    blockers: list[str] = []
    ...
    if not tactical or tactical.get("status") == "UNAVAILABLE": blockers.append("TACTICAL_EVIDENCE")
    ...
    if state in {"HOLD", "WATCH"}:
        level = "WATCH"
    elif state == "REVIEW" or any(x in blockers for x in {"NOT_AFFORDABLE", "CLUB_LIMIT", "START_SECURITY", "CRITICAL_EVIDENCE"}):
        level = "REVIEW"
    elif h3 >= _f(cfg.get("minimum_positive_3gw_for_material"), 0.5) and h5 >= _f(cfg.get("lean_gain_5gw"), 3.0):
        level = "ACTIONABLE_CHANGE" if not blockers and state == "STRONG_TRANSFER" else "MATERIAL_UPGRADE"
    else:
        level = "REVIEW"
    reason = {
        "WATCH": "candidate retained for observation; no governed transfer trigger",
        "REVIEW": "candidate merits review but one or more transfer gates are incomplete",
        "MATERIAL_UPGRADE": "3-5 GW edge is material but at least one actionability gate remains",
        "ACTIONABLE_CHANGE": "multi-GW edge and all affordability, security, tactical and timing gates are satisfied",
    }[level]
...
def _compare(out: dict[str, Any], incoming: dict[str, Any], challenger_type: str, triggers: list[str], sustainable: bool, owned: list[dict[str, Any]], itb: int, packages: dict[tuple[int, int], dict[str, Any]], external: dict[str, Any]) -> dict[str, Any]:
    ...
    package = packages.get((_i(out.get("element")), _i(incoming.get("element")))); h5 = horizons.get("5") or {}
    edge = _f(h5.get("projected_edge")); unc = _f(h5.get("combined_uncertainty")); snr = edge / unc if unc > 1e-9 else 0.0
    start_p = _f((incoming.get("xmins") or {}).get("start_probability")); evidence = _critical_evidence(out, incoming, affordable, club_legal, external)
    missing = [k for k, v in evidence.items() if v in {"UNAVAILABLE", "PARTIAL"}]; constraints_failed = [k for k, v in evidence.items() if v == "CONSTRAINT_FAILED"]
    state = "HOLD"
    ...
    row = {
        ...
        "challenger_type": challenger_type, "state": state, "horizons": horizons, "strategic_context": strategic,
        ...
        "price_urgency": "MODEL_CONTEXT_ONLY", "external_consensus": _external_state(external, _i(incoming.get("element"))), "emerging_triggers": triggers,
        ...
        "uncertainty": {"signal_to_noise_5gw": round(snr, 3), "combined_5gw": unc}, "critical_evidence": evidence, "missing_critical_evidence": missing,
        "confidence": "LOW" if missing else (incoming.get("projection_confidence") or "MEDIUM"), "advisory_only": True,
    }
    row["actionability"] = _actionability(row)
    row["reason"] = row["actionability"]["reason"]
    return row


def build() -> dict[str, Any]:
    cfg = load_policy(); projections = read_json(DATA / "projections.json", {}); team = read_json(DATA / "team.json", {}); watchlist = read_json(DATA / "dss_watchlist.json", {})
    package_optimizer = read_json(DATA / "package_optimizer.json", {}); external = read_json(DATA / "external_consensus.json", {}); pmap = _projection_map(projections)
    owned = _owned(team, pmap); owned_ids = {int(row["element"]) for row in owned}; governed_ids = _watchlist_ids(watchlist); packages = _package_map(package_optimizer); itb = _team_itb(team)
    comparisons: list[dict[str, Any]] = []
    for element in governed_ids:
        incoming = pmap.get(element)
        if not incoming: continue
        targets = [row for row in owned if row.get("position") == incoming.get("position")]; targets.sort(key=lambda row: _horizon(row.get("projection") or {}, 5)[0])
        for out in targets[: _i((cfg.get("owned_targeting") or {}).get("max_owned_targets_per_challenger"), 3)]: comparisons.append(_compare(out, incoming, "GOVERNED_WATCHLIST", [], True, owned, itb, packages, external))
    ...
    for incoming, triggers, sustainable in emerging:
        targets = [row for row in owned if row.get("position") == incoming.get("position")]; targets.sort(key=lambda row: _horizon(row.get("projection") or {}, 5)[0])
        for out in targets[: _i((cfg.get("owned_targeting") or {}).get("max_owned_targets_per_challenger"), 3)]: comparisons.append(_compare(out, incoming, "EMERGING_CHALLENGER", triggers, sustainable, owned, itb, packages, external))
    rank = {"STRONG_TRANSFER": 5, "LEAN_TRANSFER": 4, "REVIEW": 3, "WATCH": 2, "HOLD": 1}; comparisons.sort(key=lambda row: (rank.get(str(row.get("state")), 0), _f(((row.get("horizons") or {}).get("5") or {}).get("projected_edge"))), reverse=True)
    result = {
        "schema_version": 3, "contract": "OWNED_CHALLENGER_COMPARATOR_V3", "generated_at": iso_now(), "owner": cfg.get("owner"), "status": "READY", "advisory_only": True,
        "owned_count": len(owned), "governed_watchlist_count": len(governed_ids), "emerging_candidate_count": len(emerging), "comparison_count": len(comparisons),
        "state_counts": {state: sum(1 for row in comparisons if row.get("state") == state) for state in cfg.get("output_states") or []},
        "actionability_counts": {state: sum(1 for row in comparisons if (row.get("actionability") or {}).get("level") == state) for state in cfg.get("actionability_states") or []},
        "top_comparisons": comparisons[:40], "governance": cfg.get("governance"),
    }
    return result


def run() -> dict[str, Any]:
    result = build(); atomic_json(OUT, result); latest = read_json(DATA / "latest.json", {})
    latest["owned_challenger_comparator"] = {"status": result.get("status"), "owned_count": result.get("owned_count"), "governed_watchlist_count": result.get("governed_watchlist_count"), "emerging_candidate_count": result.get("emerging_candidate_count"), "comparison_count": result.get("comparison_count"), "state_counts": result.get("state_counts"), "actionability_counts": result.get("actionability_counts"), "advisory_only": True}
    latest.setdefault("files", {})["owned_challenger_comparator"] = "data/owned_challenger_comparator.json"; atomic_json(DATA / "latest.json", latest)
    print(json.dumps(latest["owned_challenger_comparator"], ensure_ascii=False)); return result
