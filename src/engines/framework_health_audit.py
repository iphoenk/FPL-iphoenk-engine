...
def _probe_universe() -> tuple[bool, dict]:
    u = read_json(DATA / "universe.json", {})
    players = list(u.get("players") or [])
    if not players:
...
def _probe_defcon() -> tuple[bool, dict]:
    try:
        from src.rules import DC_RULES, DC_POINTS_CAP_PER_MATCH, RULESET_ID
        ok = DC_RULES[1]["eligible"] is False and DC_RULES[2]["threshold"] == 10 and DC_RULES[3]["threshold"] == 12 and DC_POINTS_CAP_PER_MATCH == 2
...
def _probe_price() -> tuple[bool, dict]:
    ...
    if health:
        ok = health.get("status") == "LIVE" and bool(players)
        return ok, {"predictor_status": health.get("status"), "players": len(players)}
...
def _probe_source_health() -> tuple[bool, dict]:
    ...
    ok = all(states.get(k) == "LIVE" for k in critical)
    return ok, {"critical_endpoints": states}
...
def _probe_chip() -> tuple[bool, dict]:
    try:
        from src.rules import CHIP_RULES, RULESET_ID
        chips = read_json(DATA / "chips.json", {})
        ok = CHIP_RULES.get("one_chip_per_gameweek") is True and chips.get("ruleset_id") == RULESET_ID
...
def _probe_structural() -> tuple[bool, dict]:
    try:
        ...
        return len(rows) == 15 and legal_counts(rows), {"players": len(rows)}
    except Exception as exc:
        return False, {"error": str(exc)}
...
def _gate0(phase: str) -> dict:
    ...
    checks["G0-02"] = ("PASS" if pos.get("GK") == 2 else "FAIL", f"GK={pos.get('GK', 0)}")
    checks["G0-03"] = ("PASS" if pos.get("DEF") == 5 else "FAIL", f"DEF={pos.get('DEF', 0)}")
    checks["G0-04"] = ("PASS" if pos.get("MID") == 5 else "FAIL", f"MID={pos.get('MID', 0)}")
    checks["G0-05"] = ("PASS" if pos.get("FWD") == 3 else "FAIL", f"FWD={pos.get('FWD', 0)}")
    ...
    ids = [int(p.get("element") or -1) for p in players]
    ...
    if lineup:
        ...
        checks["G0-14"] = ("PASS" if (lineup.get("chip_context") or {}).get("single_chip_rule_respected") is True else "FAIL", "single-chip rule")
    else:
    ...
    return {"phase": phase, "counts": dict(counts), "items": items, "pass": counts.get("FAIL", 0) == 0}
...
def audit(phase: str = "preflight", strict: bool = False) -> dict[str, Any]:
    ...
    if not registry_integrity or not gate0["pass"] or critical_failed or not rules_ok or not source_ok:
        overall = "RED"
    elif critical_partial or not data_ok or any(x["counts"].get("PARTIAL", 0) for x in health.values()):
        overall = "AMBER"
    else:
        overall = "GREEN"
    ...
    out = {
        ...
        "auditor": "framework-health-auditor-v1",
        "phase": phase,
        "overall": overall,
        ...
        "rules_compliance": {"status": "PASS" if rules_ok else "FAIL", "detail": rules_detail},
