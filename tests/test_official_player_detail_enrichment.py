...
def test_fast_and_live_profiles_are_cache_only(monkeypatch, tmp_path):
    fast = _module(monkeypatch, tmp_path, "fast_decision")
    assert fast._batch_limit(623) == 0
    live = _module(monkeypatch, tmp_path, "live")
    assert live._batch_limit(623) == 0


def test_full_and_deep_profiles_are_bounded(monkeypatch, tmp_path):
    full = _module(monkeypatch, tmp_path, "full_refresh")
    assert full._batch_limit(623) == 160
    deep = _module(monkeypatch, tmp_path, "deep_stats")
    assert deep._batch_limit(623) == 623
