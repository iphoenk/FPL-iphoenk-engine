...
def test_league_snapshot_tracks_rank_gap_and_delta(monkeypatch):
    ...
    payload = {"league": {"name": "Test League"}, "standings": {"results": [
...
def test_tracking_without_explicit_or_discovered_private_league_is_explicit(monkeypatch):
    monkeypatch.setattr(oe, "_configured_league_ids", lambda kind, entry=None: [])
    monkeypatch.setattr(oe, "_discovered_private_league_ids", lambda kind, entry=None: [])
    monkeypatch.setattr(oe, "_mini_league_config", lambda: {"model_id": "mini_league_tracking_v2_autodiscovery"})
    out = oe._mini_league_tracking({"classic": {}, "h2h": {}}, {}, {})
    assert out["status"] == "NO_PRIVATE_LEAGUES_DISCOVERED"
    assert out["leagues"] == {}
    assert "no explicit or auto-discovered private mini league" in out["note"]


def test_tracking_history_is_bounded(monkeypatch):
    monkeypatch.setattr(oe, "TEAM_ID", 42)
    monkeypatch.setattr(oe, "_configured_league_ids", lambda kind, entry=None: ["99"] if kind == "classic" else [])
    monkeypatch.setattr(oe, "_discovered_private_league_ids", lambda kind, entry=None: [])
    monkeypatch.setattr(oe, "_mini_league_config", lambda: {"model_id": "mini_league_tracking_v2_autodiscovery", "history_limit": 2, "governance": {"full_rival_picks_not_collected": True}})
    payload = {"league": {"name": "Test League"}, "standings": {"results": [
        {"entry": 42, "rank": 1, "last_rank": 1, "total": 100, "entry_name": "Mine"},
        {"entry": 3, "rank": 2, "last_rank": 2, "total": 90, "entry_name": "Below"},
    ]}}
    previous = {"mini_league_tracking": {"leagues": {"classic:99": {"current": {"total_points": 99}, "history": [{"rank": 3}, {"rank": 2}]}}}}
    out = oe._mini_league_tracking({"classic": {"99": payload}, "h2h": {}}, previous, {"summary_event_points": 7})
