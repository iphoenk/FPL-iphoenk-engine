...
LEGACY_ANALYTICAL_FILENAMES = (
    "exposure.json",
    "standings_or_points.json",
    "transitions.json",
)
LEGACY_LONGITUDINAL_ANALYTICS = (
...
def _cached_history_result(manager: dict[str, Any], requested_gws: list[int]) -> dict[str, Any] | None:
    ...
    if not isinstance(expected, str) or expected != digest(
        {key: value for key, value in manager.items() if key != "record_digest"}
    ):
        return None
    by_gw = {
        int(row["gw"]): row
    ...
    *,
    ...
    picks["official_exclusion_reason_counts"] = {
        BEFORE_FIRST_OFFICIAL_ENTRY_HISTORY_GW: sum(
            ...
            if item.get("official_exclusion_reason") == BEFORE_FIRST_OFFICIAL_ENTRY_HISTORY_GW
        )
    }
...
def _event_points(result: dict[str, Any], *, gw: int, completed: bool) -> dict[str, Any]:
    ...
    if result.get("status") == "LIVE":
        for item in ((result.get("payload") or {}).get("elements") or []):
            ...
            if (
                ...
                elements.append(
                    {
                        "official_element_id": int(item["id"]),
    ...
    return {
        ...
        "status": "AVAILABLE" if elements else "UNAVAILABLE",
        "elements": sorted(elements, key=lambda row: row["official_element_id"]),
        "authority": "OFFICIAL_FPL",
...
def _reconciliation(
    ...
    *,
    ...
    checks = {
        ...
        "captain_multiplier_consistent": (
            captain_multiplier_consistent(picks) if pick_record.get("status") == "AVAILABLE" else None
        ),
...
def _manager_history(
    ...
    for entry_id in manager_ids:
        ...
        manager = {
            ...
            "gws": [
                _manager_gw_fact(
                    (picks_by_gw[gw].get("entries") or {}).get(str(entry_id), {}),
                    history_result,
                    gw,
    ...
    return {
        ...
        "cohort_semantics": COHORT_SEMANTICS,
        "managers": managers,
        "authority": "OFFICIAL_FPL_NORMALIZED_FACT",
    }
...
def _retire_legacy_analytics(history_root: Path, gws: list[int]) -> list[str]:
    removed: list[str] = []
    for gw in gws:
        gw_root = history_root / f"gw_{gw}"
        for name in LEGACY_ANALYTICAL_FILENAMES:
            path = gw_root / name
            if path.exists():
                path.unlink()
                removed.append(str(path.relative_to(history_root)))
    longitudinal_root = history_root / "longitudinal"
    for name in LEGACY_LONGITUDINAL_ANALYTICS:
        path = longitudinal_root / name
        if path.exists():
            path.unlink()
            removed.append(str(path.relative_to(history_root)))
...
class HistoricalBackfillService:
    ...
    def _write(self, path: Path, payload: dict[str, Any]) -> None:
        assert_publish_safe(payload, secret_values=getattr(self.client, "secret_values", ()))
    ...
    def run(
        ...
        *,
        ...
        if entry_result.get("status") != "LIVE":
            raise HistoricalBackfillError(
                "Official entry endpoint unavailable for priority league resolution"
            )
        ...
        if (
            not target
            or target.get("resolution_status") != "RESOLVED"
            or target.get("league_id") is None
        ):
        ...
        if str(target.get("league_kind") or "").lower() != "classic":
            raise HistoricalBackfillError(
                "historical mini-league backfill currently requires a classic priority league"
            )
        if not bool(target.get("full_submitted_picks")):
            raise HistoricalBackfillError(
                "priority league is not configured for full submitted picks"
            )
        ...
        if not standings.get("complete"):
            raise HistoricalBackfillError(
                "current priority league standings incomplete; refusing ambiguous cohort"
            )
        ...
        previous_manager_history = (
            read_json(history_root / "longitudinal" / "manager_history.json")
            if cache_enabled
            else None
        )
        ...
        telemetry = {
            ...
            "event_live_requests": 0,
            "retry_count": (
                _retry_count(bootstrap_result)
                + _retry_count(entry_result)
                + history_metrics["retry_count"]
            ),
        ...
        for gw in gws:
            ...
            telemetry["event_live_requests"] += 1
            telemetry["retry_count"] += _retry_count(live_result)
            event_points = _event_points(live_result, gw=gw, completed=completed)
            event_points_available[gw] = event_points["status"] == "AVAILABLE"
            ...
            reconciliations_by_gw[gw] = reconciliations

            self._write(gw_root / "manager_picks.json", picks)
            self._write(gw_root / "event_points.json", event_points)
            self._write(
                gw_root / "entry_history.json",
                {
                    "schema_version": HISTORICAL_SCHEMA_VERSION,
                    "semantic_class": "NORMALIZED_FACT",
                    "canonical": True,
        ...
        manager_history = _manager_history(
            manager_ids,
            picks_by_gw,
            histories,
            gws,
        ...
        self._write(history_root / "longitudinal" / "manager_history.json", manager_history)
        ...
        for gw in gws:
            ...
            history_available_ids = {
                row["entry_id"] for row in reconciliations if row["history_available"]
            }
            ...
            reconciliation_failures = {
                ...
                if row["entry_id"] in eligible_ids
                and row["history_available"]
                and not row["consistent"]
            }
            history_missing = eligible_ids - history_available_ids
            failed_ids = sorted(
                pick_failures
                | reconciliation_failures
                | (history_missing if completed else set())
            )
            ...
            raw_coverage_count = (
                min(available, raw_history_available_count) if completed else available
            )
            eligible_coverage_count = (
                min(
                    len(available_ids & eligible_ids),
                    len(history_available_ids & eligible_ids),
                )
                if completed
            ...
            health = {
                ...
                "official_exclusion_reason_counts": dict(
                    picks.get("official_exclusion_reason_counts") or {}
                ),
                ...
                "entry_history_missing_count": len(manager_ids)
                - raw_history_available_count,
                "eligible_entry_history_available_count": len(
                    history_available_ids & eligible_ids
                ),
                "eligible_entry_history_missing_count": len(history_missing),
                "entry_history_required_for_complete": completed,
                "entry_history_current_gw_policy": (
                    None
                    if completed
                    else "OPTIONAL_UNTIL_OFFICIAL_CURRENT_GW_HISTORY_IS_AVAILABLE"
                ),
                ...
                "coverage_percent": round(
                    raw_coverage_count * 100 / len(manager_ids), 4
                ),
                "eligible_coverage_percent": (
                    round(eligible_coverage_count * 100 / eligible_count, 4)
                    if eligible_count
                    else 100.0
                ),
                "complete": complete,
                "complete_with_explicit_official_exclusions": (
                    complete and bool(excluded_ids)
                ),
                "failed_entry_ids": failed_ids,
                "officially_unavailable_or_optional_entry_ids": (
                    sorted(history_missing) if not completed else []
                ),
            }
        ...
        managers_artifact = {
            ...
            "membership_semantics": (
                "Current standings membership is authoritative only for the current cohort; "
                "historical membership per GW is unknown unless Official FPL supplies separate evidence."
            ),
        ...
        self._write(history_root / "managers.json", managers_artifact)

        retired = _retire_legacy_analytics(history_root, gws)

        complete_gws = sum(1 for row in gw_health if row["complete"])
        partial_gws = sum(
            1
            for row in gw_health
            if not row["complete"] and row["eligible_coverage_percent"] > 0
        )
        failed_gws = len(gw_health) - complete_gws - partial_gws
        overall_status = (
            "GREEN"
            if complete_gws == len(gws)
            else ("AMBER" if complete_gws or partial_gws else "RED")
        )

        client_telemetry = (
            self.client.telemetry()
            if callable(getattr(self.client, "telemetry", None))
            else {}
        )
        ...
        manifest = {
            ...
            "completed_requested_gw_count": sum(
                1 for gw in gws if gw_states[gw]["finished"]
            ),
            "provisional_current_gw_count": sum(
                1 for gw in gws if not gw_states[gw]["finished"]
            ),
            ...
            "cache": {
                ...
                "current_gw_entry_history_cache_reused": (
                    False
                    if has_provisional_current
                    else telemetry["history_cache_hits"] > 0
                ),
                "force": bool(force),
            ...
            "publication_contract": {
                "canonical_atomic_facts_only": True,
                "mini_league_analytics_authority": "NONE",
                "retired_analytical_artifacts": list(LEGACY_ANALYTICAL_FILENAMES)
                + list(LEGACY_LONGITUDINAL_ANALYTICS),
                "removed_from_runtime_tree": retired,
            },
            "lineage": {
            ...
            "limitations": [
                (
                    "Historical league membership is not inferred from current membership; "
                    "records are CURRENT_COHORT_HISTORY."
                ),
                (
                    "Historical league rank is not reconstructed or asserted inside V6; "
                    "rank analytics are downstream-only."
                ),
                (
                    "Historical club identity is not asserted when Official historical endpoints "
                    "do not expose it; element_id remains authoritative."
                ),
                (
                    "Completed-GW current-cohort entries may be explicitly excluded only when "
                    "Official submitted-picks returns 404 and Official entry-history begins in a later GW; "
                    "this is an availability fact and not historical league-membership evidence."
                ),
                (
                    "A current post-deadline GW is provisional: submitted picks are factual, "
                    "but event points are labeled live rather than final and Official entry-history "
                    "rows may remain unavailable until the GW completes."
                ),
                (
                    "Ownership, EO, transitions, pairwise overlap, concentration/HHI, reconstructed "
                    "cohort ranks, and behavioral labels are not authored or published by V6."
                ),
            ],
        }
        self._write(history_root / "manifest.json", manifest)
        self._write(self.output_root / "health" / "historical_backfill.json", manifest)
        return manifest


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Governed V6 factual historical mini-league backfill"
    )
    ...
    try:
        ...
        print(
            json.dumps(
                {"status": "RED", "error": safe_error(exc), "reason": str(exc)},
                indent=2,
            )
        )
        return 2
