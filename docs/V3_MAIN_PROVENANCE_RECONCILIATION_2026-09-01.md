...
## Current first-parent state

PR #303 has since merged on top of the direct state commit. Its merge commit has the direct state commit as first parent, so re-anchoring to the audited state commit preserves the intended rule: PR #303 and every subsequent first-parent commit must verify as a merged PR before the walk reaches the reconciled anchor.
...
3. The merge commit must resolve to this merged PR; PR #303 must also resolve as a merged PR.
