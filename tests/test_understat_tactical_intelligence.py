...
def test_canonical_team_route_requires_correlated_metrics_and_ppda_is_style_only():
    evidence = {
        "windows": {
            "last_5": {
                "matches": 5,
                "metrics_adjusted_per_match": {"xga": 1.8, "deep_allowed": 11.0, "xg": 1.1, "deep": 5.0, "ppda": 7.0},
                "league_mean": {"xga": 1.2, "deep_allowed": 7.0, "xg": 1.3, "deep": 7.0, "ppda": 10.0},
            }
        }
    }
    routes = context._team_route_context(evidence)
    assert "box_pressure" in routes["vulnerabilities"]
    assert "final_third_progression" in routes["vulnerabilities"]
    assert "high_press_activity_proxy" in routes["style_proxies"]
    assert "transition_threat" not in routes["vulnerabilities"]


def test_player_metrics_extend_existing_return_route_vocabulary():
    player = {
        "season_to_date": {
            "metrics": {
                "xg": {"value": 1.2},
    ...
    routes = context._player_return_routes(player)
    assert set(routes) == {"box_pressure", "chance_creation", "final_third_progression", "shot_volume"}
