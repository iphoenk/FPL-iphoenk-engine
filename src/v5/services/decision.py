...
def _prepare(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    watchlist = build_watchlist(prediction, team)
...
def _blocked_trace(reason: str, gate0_preflight: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "decision_type": "BLOCKED", "action": reason, "confidence": "LOW",
        "evidence": [{"source": "governance-service", "field": "gate0_preflight", "authority": "governance-service", "freshness": None, "provenance": {"model": gate0_preflight.get("model"), "pass": gate0_preflight.get("pass")}}] if gate0_preflight else [],
...
def _finalize(payload: dict[str, Any], prepared: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    watchlist = prepared.get("watchlist") if isinstance(prepared.get("watchlist"), dict) else {}
    ...
    dss = evaluate_dss(truth, price, prediction, local_capabilities=local_capabilities, external_capability_sources={"evaluation": evaluation_capabilities})
    ...
    if local_ready:
        trace = build_trace(truth=truth, prediction=prediction, price=price, packages=packages, package_governance=package_governance, lineup=lineup, dss=dss, gate0_preflight=gate0_preflight)
        status = "READY" if preflight_ready else "BLOCKED"
    else:
        trace = _blocked_trace("BLOCK decision output until package, package-governance and lineup authorities are READY", gate0_preflight)
        status = "BLOCKED"
    return {
        "status": status,
        "model": _cfg().get("model_id"),
        ...
        "governance": {
            **(packages.get("governance") or {}), **(package_governance.get("governance") or {}),
            "manual_authority_override": bool(package_governance.get("manual_authority_override")),
            "lineup_authority": lineup.get("authority"), "dss_evaluation_model": dss.get("evaluation_model"),
            "evaluation_capabilities_consumed": sorted(str(x) for x in evaluation_capabilities),
            "gate0_preflight_model": gate0_preflight.get("model"), "decision_trace_required": True,
            "watchlist_contract": watchlist.get("screening_contract"),
...
def handle(operation: str, payload: dict[str, Any]) -> Any:
    if operation == "status":
        return {"status": "ACTIVE", "bridge_only": False, "production_recommendation": False, "model": _cfg().get("model_id"), "capabilities": list(_cfg().get("capabilities") or []), "operations": ["prepare", "finalize", "build"]}
