...
def _require_files(cfg: dict[str, Any], data_dir: Path) -> None:
    missing = [
        str(name)
        for name in cfg.get("required_artifacts") or []
        if not (data_dir / str(name)).is_file()
    ]
    if missing:
...
def _validate_contract(
    ...
    if age_seconds > max_age:
        raise RuntimeError(
            f"REFRESH_REQUIRED: materialized decision is stale ({age_seconds:.1f}s > {max_age}s for {phase_name})"
        )
    return {
        "phase_freshness_policy": phase_name,
