...
def _tactical_overlay(payload:dict[str,Any],prediction:dict[str,Any])->dict[str,Any]:
    pmap={int(row["element"]):row for row in prediction.get("players") or [] if isinstance(row,dict) and row.get("element") is not None}
    positions={}
    reranked=0
    for position,raw_rows in (payload.get("positions") or {}).items():
        ...
        rows=close_group_sort(rows,score=lambda row:float(row.get("score") or 0),player=lambda row:pmap.get(int(row.get("element") or -1),{}),gap=watchlist_gap())
        after=[int(row.get("element") or -1) for row in rows]
        if before!=after: reranked+=1
        for rank,row in enumerate(rows,start=1):
            row["rank"]=rank
            player=pmap.get(int(row.get("element") or -1),{})
            row["tactical_matchup"]=compact_tactical(player)
        positions[str(position)]=rows
    return {**payload,"positions":positions,"governance":{**(payload.get("governance") or {}),"tactical_consumption_contract":"TACTICAL_DECISION_CONSUMPTION_V1","tactical_tiebreak_close_screened_pool_only":True,"tactical_membership_promotion_forbidden":True,"tactical_reranked_position_count":reranked}}


def handle(operation:str,payload:dict[str,Any])->Any:
    if operation=="status": return {"status":"ACTIVE","model":"full_dss_watchlist_v5_v3_tactical_consumption","operations":["build"],"tactical_consumption":"CLOSE_CALL_ONLY"}
