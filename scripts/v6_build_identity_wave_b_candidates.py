...
Current safe migration route for Understat:

    Official FPL bootstrap.elements[].code
      == frozen Reep-v0 people.csv key_opta_numeric
      -> frozen verified key_transfermarkt
      == Reep-v1 canonical bridge transfermarkt/spi(e)ler
      -> Reep-v1 stable entity
      -> Reep-v1 canonical bridge understat/player
      -> Understat native player id

Why the v0 hop still exists:
Reep-v1 opta_numeric is an overlay/discovery surface, not canonical bridge
evidence. v0 and v1 Reep IDs are explicitly not interchangeable. Therefore we
migrate through a typed provider ID (Transfermarkt player ID), never through a
Reep-ID equality assumption.
...
METHOD = "REEP_V0_OPTA_TO_TRANSFERMARKT__REEP_V1_TRANSFERMARKT_TO_UNDERSTAT"
...
def _official_codes(bootstrap: dict[str, Any]) -> set[int]:
    ...
    for row in rows:
        ...
        codes.add(code)
    if duplicate:
        raise WaveBCandidateError(f"duplicate Official FPL codes: {sorted(duplicate)[:10]}")
    if not codes:
        raise WaveBCandidateError("Official FPL bootstrap exposes no usable codes")
    return codes
...
def _read_v1_bridge_indexes(path: Path) -> tuple[dict[str, str], dict[str, str]]:
    """Return Transfermarkt player id -> Reep-v1 id and Reep-v1 id -> Understat id."""
    try:
        handle = path.open("r", encoding="utf-8", newline="")
    except OSError as exc:
    ...
    with handle:
        ...
        for row in reader:
            ...
            if external_id is None or reep_id is None:
                continue
            if provider == "transfermarkt" and namespace in {"spieler", "player"}:
    ...
    return tm_to_reep, reep_to_understat
...
def build_understat_candidates(
    *,
    ...
    tm_to_reep, reep_to_understat = _read_v1_bridge_indexes(reep_v1_bridges)
    ...
    for code, tm in sorted(official_to_tm.items()):
        ...
        candidates.append(
            {
                "source_id": "understat",
                "source_native_id": int(understat_native) if understat_native.isdigit() else understat_native,
    ...
    return {
        ...
        "governance": {
            ...
            "typed_transfermarkt_hub_required": True,
