...
def test_composite_release_gate_preserves_all_underlying_integrations():
    ...
    assert names == [
        ...
        "full_resource_guard",
        "fast_semantic_warmup",
        "warmup_source_contract",
        "warmup_production_contract",
        "warmup_watchlist_contract",
        "warmup_report_serving_contract",
        "capture_reuse_manifest",
        "fast_runtime",
        "fast_slo_guard",
    ]
    commands = [" ".join(gate.command) for gate in gates]
    assert any("orchestrator --mode daily --stats --profile full_refresh" in command for command in commands)
    ...
    assert sum("fast_entrypoint --mode daily --stats --profile fast_decision" in command for command in commands) == 2
    assert any("reuse_manifest --profile fast_decision" in command for command in commands)
