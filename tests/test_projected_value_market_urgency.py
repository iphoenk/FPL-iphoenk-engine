...
def test_identity_team_or_position_mismatch_repairs_display_but_blocks_projection(monkeypatch):
    player = _projection(101, 18.0, 40)
    ...
    blocked = result["blocked_identity"][0]
    assert blocked["reason"] == "IDENTITY_MAPPING_MISMATCH_REPAIRED_FROM_OFFICIAL"
    assert blocked["downstream_projection_trusted"] is False
