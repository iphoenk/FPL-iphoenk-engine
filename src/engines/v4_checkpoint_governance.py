...
def _plain_reasoning(action: str, verdict: str, freshness: dict, lineup: dict, planning_scorecard: dict, recommended: dict, wildcard_active: bool) -> list[str]:
    ...
    if action == "REFRESH_REQUIRED":
        return ["Data terakhir sudah terlalu lama untuk dijadikan dasar keputusan saat ini.", "Perbarui kondisi pemain, harga, dan informasi tim sebelum melakukan perubahan."]
    if action == "BLOCKED":
        return ["Data atau struktur tim belum konsisten untuk menghasilkan keputusan yang aman.", "Jangan mengubah tim sampai sumber masalah sudah diperbaiki dan pemeriksaan ulang selesai."]
    ...
    if action == "REVIEW_REQUIRED":
        return ["Ada kandidat perubahan yang layak dibandingkan kembali sebelum keputusan final.", "Manfaatnya belum cukup kuat untuk langsung dijalankan tanpa mengecek starter security, team news, dan horizon fixture terbaru."]
    if action == "GO":
...
def _emission_contract(context: dict) -> dict:
    ...
    if context.get("post_final_emergency_only") is True:
        raise RuntimeError("legacy post_final_emergency_only is forbidden; checkpoint_policy is the sole timing authority")
    authorized = bool(context.get("visible_output_authorized"))
    if context.get("duplicate_report_forbidden") is False:
    ...
    if policy_id in {"DEADLINE_MONITOR", "FINAL_DEADLINE_REVIEW"} and (not authorized or not full_required or not no_change_required):
    ...
    return {
        ...
        "visible_report_count": 1 if authorized else 0,
...
def govern_checkpoint(latest: dict, health: dict, sanity: dict, lineup: dict, locked: dict, scorecard: dict | None = None, now: datetime | str | None = None, actions: dict | None = None) -> dict:
    evaluated_at = parse_dt(now) if isinstance(now, str) else now
    ...
    reasons: list[str] = []
    if not gate0_pass: reasons.append("GATE0_FAILED")
    if framework_red: reasons.append("FRAMEWORK_RED")
    if not authority_ok: reasons.append("SQUAD_AUTHORITY_MISMATCH")
    if not freshness["pass"]: reasons.append("SNAPSHOT_STALE")
    if simulation: reasons.append("SIMULATED_AS_OF")
    if health.get("critical_partial"): reasons.append("CRITICAL_FRAMEWORK_PARTIAL")
    if health.get("critical_warmup"): reasons.append("CRITICAL_PREDICTION_WARMUP")
    if verdict == "OPTIONAL_IMPROVEMENT": reasons.append("OPTIONAL_NOT_AUTOMATIC_GO")
    ...
    return {
        ...
        "human_report": {
            "language_policy": language_policy.get("registry"), "audience": language_policy.get("audience", "FPL_MANAGER"), "decision": action,
            "headline": action_definition.get("headline"), "summary": action_definition.get("summary"), "why": human_reasoning, "what_to_do": human_actions,
            "technical_terms_suppressed_from_primary_reasoning": True, "technical_state_location": language_policy.get("technical_state_location"),
        },
        "squad": {
            "authority": latest.get("squad_authority"), "expected_authority": expected_authority, "authority_ok": authority_ok,
            "baseline_gw": authority.get("baseline_gw"), "planning_gw": authority.get("planning_gw"), "planning_override_applied": authority.get("override_applied"),
            "planning_override_target_gw": authority.get("override_target_gw"), "authority_source": authority.get("authority_source"), "wildcard_active": wildcard_active,
            "locked_players": len(locked.get("players") or []), "composition_status": "LOCKED_15" if expected_authority == "LOCKED_PRE_DEADLINE" else "SUBMITTED_OR_CURRENT",
            "hit_recommendation": "NOT_APPLICABLE_WILDCARD_ACTIVE" if wildcard_active else "UNASSESSED",
        },
        "decision": {
            "raw_package_verdict": sanity.get("raw_package_verdict"), "governed_verdict": verdict, "recommended_replacements": recommended.get("replacements"),
            "recommended_out": [row.get("name") for row in recommended.get("out", [])], "recommended_in": [row.get("name") for row in recommended.get("in", [])],
            "material_eligible": recommended.get("material_eligible"), "engine_is_advisory": True, "user_decision_is_final_authority": True, "execution_authorized": execution_authorized,
        },
        "lineup": {
            "status": lineup_state, "decision_authority": lineup.get("authority") or lineup.get("decision_authority") or "ENGINE_RECOMMENDATION", "formation": lineup.get("formation"),
            "captain": (lineup.get("captain") or {}).get("name"), "vice_captain": (lineup.get("vice_captain") or {}).get("name"), "active_chip": (lineup.get("chip_context") or {}).get("active_chip"),
            "human_override_active": bool(planning_scorecard.get("human_override_active")), "engine_comparison": planning_scorecard.get("engine_comparison") or {}, "requires_explicit_final_lock": not explicit_lineup_lock,
        },
        "personal_gw_scorecard": {
            "status": scorecard.get("status", "UNAVAILABLE"), "previous_gw": scorecard.get("previous_gw") or {"status": "UNAVAILABLE"},
            "planning_gw": planning_scorecard or {"status": "UNAVAILABLE"}, "headline": scorecard.get("headline") or {}, "history": scorecard.get("history") or [],
        },
        "readiness": {
            "framework_health": health.get("overall"), "pipeline_health": health.get("pipeline_health", health.get("overall")), "prediction_health": health.get("prediction_health"),
            "decision_engine": health.get("decision_engine"), "capability_coverage": health.get("capability_coverage"), "gate0_pass": gate0_pass, "health_go_allowed": health_go,
            "freshness": freshness, "critical_partial": critical_partial, "critical_warmup": critical_warmup, "reasons": reasons,
        },
        "report_scope": list(emission.get("report_scope") or []),
        "guardrails": {
            "raw_optimizer_not_authoritative": True, "optional_improvement_is_not_automatic_go": True, "simulation_never_authorizes_action": True,
            "freshness_failure_blocks_action": True, "locked_15_separate_from_lineup_lock": True, "wildcard_active_means_no_hit": True, "scorecard_is_reporting_only": True,
            "planning_authority_target_gw_aware": True, "stale_wildcard_flag_does_not_force_future_lock": True, "engine_is_advisory": True, "user_decision_is_final_authority": True,
            "go_never_auto_executes_without_user_final_lock": True, "primary_report_plain_fpl_language": True, "technical_reason_codes_separate_from_human_reasoning": True,
            "checkpoint_policy_is_sole_timing_authority": True, "legacy_post_final_emergency_path_forbidden": True, "visible_report_count_never_exceeds_one": True,
...
def run(now: str | None = None) -> dict:
    ...
    out = govern_checkpoint(
        read_json(DATA / "latest.json", {}), read_json(DATA / "framework_health_v4.json", {}), read_json(DATA / "recommendation_sanity_v4.json", {}),
        effective_plan, read_json(CONFIG / "locked_squad.json", {}), scorecard=read_json(DATA / "gw_scorecard_v4.json", {}), now=now,
    )
    atomic_json(OUTFILE, out)
    print(json.dumps({
        "checkpoint": (out.get("checkpoint_context") or {}).get("policy_id"), "visible_report_count": (out.get("emission") or {}).get("visible_report_count"),
        "action": out.get("action_state"), "headline": out.get("headline"), "governed_verdict": (out.get("decision") or {}).get("governed_verdict"),
        "decision_authority": (out.get("lineup") or {}).get("decision_authority"), "previous_gw": ((out.get("personal_gw_scorecard") or {}).get("headline") or {}).get("previous"),
        "planning_gw": ((out.get("personal_gw_scorecard") or {}).get("headline") or {}).get("planning"), "squad_basis": (out.get("squad") or {}).get("authority_source"),
