...
> **Last runtime/documentation sync:** `2026-09-21T13:55:00+07:00`  
...
A mandatory DEEP occurrence now has one explicit integrated execution path rather than relying on report prose to opportunistically call disconnected libraries. The non-scheduled `V12 integrated report runner` binds a same-occurrence V6 `full_master` report-prefetch to the existing V12 owners, executes the supportable P1.1/P1.3/P1.6/P1.7/price/ICON surfaces, materializes the exact Canonical DEEP catalog, and emits an occurrence-bound `report_bundle.json`, `report_body.md` and `execution_proof.json`. It creates no scheduler, never publishes decisions into `runtime-data-v6`, and may not import/execute V3/V4/V5 or legacy optimizer/oracle paths. FPL Master Monitor V12 remains the only recurring scheduler and consumes the bundle for mandatory DEEP delivery. If the runner is blocked, the due report remains fail-operational but must keep every Canonical section and expose the exact blocked stage instead of collapsing into a short manual summary.

The integrated runner intentionally fails closed on the remaining full-universe composite-score gap: P1.1/P1.3/P1.6 may execute for the full player universe, but Section 14/Watchlist cannot claim full 20/25/30/25 optimization until V12-native numeric producers for PROVEN/HISTORICAL, TACTICAL/ROLE, CURRENT UNDERLYING and FIXTURE/SECURITY are all bound to `compute_football_score()`. No ad-hoc percentile score or legacy runtime fallback is permitted.
