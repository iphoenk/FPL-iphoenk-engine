...
def build_predictions(bootstrap: dict[str, Any], fixtures: list[dict[str, Any]], rules: dict[str, Any], planning_gw: int, horizon: int = 15) -> dict[str, Any]:
    ...
    for player in bootstrap.get("elements") or []:
        ...
        for gw in range(planning_gw, planning_gw + horizon):
            ...
            for matchup in [m for m in team_matchups if int(m.get("event") or -1) == gw]:
                ...
                role_attack_multiplier = clamp(
                    max(set_piece_multiplier, penalty_multiplier),
                    1.0,
                    _f(role_adjustment.get("maximum_attack_multiplier"), 1.30),
                )
                attack = (
                ...
                if len(network_fixtures) < 5:
                    network_fixtures.append({
                        ...
                        "xmins": {k: xmins.get(k) for k in ("start_probability", "bench_probability", "dnp_probability", "expected_minutes")},
                    })
            ...
            by_gw.append({
                ...
                "std": round(gw_std, 3),
                "clean_sheet_probability": round(1 - math.prod(1 - _f(x.get("clean_sheet_probability")) for x in details), 4) if details else 0.0,
                "fixtures": details,
            })
        horizons = {}
        for h in (3, 5, 10, 15):
    ...
    return {
        "generated_at": _now(),
        "schema_version": 511,
        "model_version": str(cfg.get("model_id") or "player_projection_v310"),
