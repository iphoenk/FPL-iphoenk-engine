...
def _emission_contract(context):
    if context.get("post_final_emergency_only") is True:raise RuntimeError("legacy post_final_emergency_only is forbidden")
...
def _plain_reasoning(canonical,freshness,lineup):
    ...
    if overall=="HOLD":reasons.append("Belum ada perubahan yang cukup kuat dan bisa dijalankan sekarang.")
    elif overall=="REVIEW":reasons.append("Ada keputusan yang perlu ditinjau lagi sebelum perubahan apa pun dijalankan.")
    else:reasons.append("Bukti saat ini mendukung perubahan yang sudah memenuhi syarat eksekusi.")
    if transfer.get("candidate_state")=="MATERIAL_UPGRADE_NON_ACTIONABLE":reasons.append("Ada challenger material, tetapi masih tertahan oleh bukti, timing, atau actionability yang belum lengkap.")
    if freshness.get("freshness_state")!="FRESH":reasons.append("Freshness sumber belum berada pada kondisi terbaik untuk eksekusi final.")
...
def _plain_actions(canonical):
    ...
    if overall=="REVIEW":return ["Jangan eksekusi perubahan dulu.","Tutup blocking reasons dan close call pada final review."]
    actions=["Pertahankan struktur saat ini.","Pantau team news, price urgency, dan challenger sampai checkpoint berikutnya."]
    if transfer.get("candidate_state")=="MATERIAL_UPGRADE_NON_ACTIONABLE":actions.append("Simpan challenger material di review board, bukan sebagai instruksi transfer.")
    return actions
