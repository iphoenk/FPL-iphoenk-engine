...
- The canonical V3 candidate topology maps 21 artifact-owned capabilities exactly once into 11 execution domains across 6 phases: ACQUIRE (`official_state`, `personal_team_state`), ENRICH (`football_context`, `market_context`), MODEL (`prediction`), DECISION (`squad_decision`, `challenger_analysis`), GOVERNANCE (`framework_governance`, `prediction_validation`) and PUBLISH (`reporting`, `serving`).
- The latest accepted production evidence below was generated by the predecessor 7-domain topology. The 11-domain topology must pass CI, merge and runtime publication before it can replace that historical production proof.
