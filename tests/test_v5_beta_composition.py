...
def test_beta_composition_adds_watchlist_comparator_reporting_and_persistence(monkeypatch):
    base={"correlation_id":"c1","team_summary":{"authority":"user_lock","owned_ids":[1],"team_value_ledger":[{"element":1}]},"phase":{"phase":"PRE_DEADLINE"},"price_summary":{"alerts":[]},"decision_summary":{"dss":{},"lineup":{},"selected_package_id":"HOLD"},"framework_health":{"go_allowed":False},"service_performance":{}}
    monkeypatch.setattr(orchestrator_beta,"core_handle",lambda operation,payload:dict(base))
    persisted=[]
    def parallel(calls,correlation_id):
        if "prediction" in calls:return {"prediction":{"data":{"planning_gw":2,"players":[]}},"report_state":{"data":{}}}
    ...
    def one(service,operation,payload,correlation_id):
        ...
        if service=="evaluation" and operation=="compare_owned_challenger":return {"data":{"model":"v5_owned_challenger_comparator_v1","status":"ACTIVE_ALPHA","authority":"ADVISORY_ONLY","pair_count":0,"classification_counts":{},"top_comparisons":[]},"elapsed_ms":1,"round_trip_ms":2}
