...
def frontier_exclusion_audit_from_candidates(
    ...
    *,
    ...
    """Expose who is retained/pruned by the existing package frontier.

    This helper is deliberately audit-only. It mirrors the exact ranking semantics
    used by ``v4_wc_package_audit.frontier`` and MUST NOT influence candidate
    generation, package ranking, beam width, legality, decision authority, or
    ...
    for position in POSITION_COUNTS:
        ...
        for rank, player in enumerate(rows[:audit_depth_per_position], start=1):
            emitted.append(
                {
                    "rank": rank,
                    "frontier_state": "IN_FRONTIER"
                    if rank <= production_frontier_per_position
                    else "PRUNED",
