...
def test_official_snapshot_fetches_are_concurrent(monkeypatch):
    import threading
    import src.services.raw_snapshot_service as raw_snapshot
    ...
    monkeypatch.setattr(raw_snapshot, "get_json", fake_get)
    out = raw_snapshot._parallel_official_get([
        ("one", "one/", 3),
        ("two", "two/", 2),
        ("three", "three/", 1),
    ])
...
def test_legal_counts():
    players = []
    for i in range(2):
        players.append({"element": i, "position": "GK", "team": i})
    for i in range(5):
        players.append({"element": 10 + i, "position": "DEF", "team": 10 + i})
    for i in range(5):
        players.append({"element": 20 + i, "position": "MID", "team": 20 + i})
    for i in range(3):
        players.append({"element": 30 + i, "position": "FWD", "team": 30 + i})
