...
def _structured_rows(payload: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        if isinstance(row, dict) and row.get("contract") == OBSERVATION_CONTRACT
    ]
...
def _disagreement_states(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for subject, items in grouped.items():
        ...
        out.append({
            "subject_key": subject,
            "player": (items[0].get("subject") or {}).get("player"),
    ...
    for spec in specs:
        ...
        for capability in spec.capabilities:
            ...
            fallback = [
                ...
                if row.get("contract") == OBSERVATION_CONTRACT
                and row.get("source_id") == spec.source_id
                and row.get("capability") == capability
                and row.get("status") in {"CACHED_LAST_KNOWN_GOOD", "STALE"}
            ]
            if state not in {"AVAILABLE", "AUTHORITATIVE_NATIVE", "INGESTED"} and fallback:
                if any(row.get("status") == "CACHED_LAST_KNOWN_GOOD" for row in fallback):
                    state = "CACHED_LAST_KNOWN_GOOD"
                else:
                    state = "STALE"
            rows.append({
                ...
                "source_reachable": result.reachable,
                "data_state": state,
                "fresh_observations": sum(
                    1 for row in reconciled
                    if row.get("contract") == OBSERVATION_CONTRACT
                    and row.get("source_id") == spec.source_id
                    and row.get("capability") == capability
...
def collect_sources(data_dir: Path) -> dict[str, Any]:
    ...
    observation_payload = {
        ...
        "cross_source": disagreement,
        "policy": {
            "official_native_fields_win": True,
            "missing_observations_are_not_fabricated": True,
            "source_reachability_is_separate_from_capability_health": True,
            "cached_or_stale_rows_are_not_current": True,
        },
    }
    return {
        "schema_version": 2,
        ...
        "policy": {
            ...
            "source_reachability_is_separate_from_capability_health": True,
            "stale_observations_are_never_silently_current": True,
        },
    }
