...
from src.models.v12_stage_b_ab import run_stage_b_controlled_ab
from src.models.v12_stage_b_evidence import build_stage_b_evidence_snapshot
...
def _stage_b_player(
    *,
    ...
    return {
        ...
        "penalties_order": 2,
        "penalties_text": "Second choice",
        "corners_and_indirect_freekicks_order": 1,
        "corners_and_indirect_freekicks_text": "Primary",
        "direct_freekicks_order": 1,
        "direct_freekicks_text": "Primary",
    }


def test_stage_b_high_defcon_defender_remains_separate_from_weak_cs():
    rows = [
        _stage_b_dc_row(gw, cbi=9, tackles=3, opponent=50 + gw)
        for gw in range(1, 7)
    ]
    context = build_defcon_context(rows)
...
def test_stage_b_low_defcon_defender_not_rescued_by_strong_cs_context():
    rows = [
        _stage_b_dc_row(gw, cbi=3, tackles=1, opponent=60 + gw)
        for gw in range(1, 7)
    ]
    context = build_defcon_context(rows)
    out = project_defcon_hit_probability(
    ...
    assert out["defcon_hits"] == 0
    assert out["projected_hit_probability"] < 0.25
    assert out["DEFCON_EV"] < 0.5


def test_stage_b_defcon_home_away_asymmetry_is_sample_aware():
    rows = [
        *[
            _stage_b_dc_row(gw, home=True, cbi=10, tackles=3)
            for gw in range(1, 4)
        ],
        *[
            _stage_b_dc_row(gw, home=False, cbi=3, tackles=2)
            for gw in range(4, 7)
        ],
    ]
    context = build_defcon_context(rows)
    ...
    assert home["home_hit_rate"] == 1.0
    assert home["away_hit_rate"] == 0.0
    assert (
        home["conditions"]["home_away"]["status"]
        == away["conditions"]["home_away"]["status"]
        == "APPLIED"
    )
    assert home["projected_hit_probability"] > away["projected_hit_probability"]


def test_stage_b_defcon_under_three_starts_stays_low_confidence():
    rows = [
        _stage_b_dc_row(1, cbi=10, tackles=3),
        _stage_b_dc_row(2, cbi=10, tackles=3),
    ]
    out = project_defcon_hit_probability(
        rows,
        position="DEF",
...
def test_stage_b_defcon_probability_responds_to_xmins_uncertainty():
    rows = [
        _stage_b_dc_row(gw, cbi=9, tackles=3)
        for gw in range(1, 7)
    ]
    context = build_defcon_context(rows)
    high = project_defcon_hit_probability(
        rows,
        position="DEF",
        xmins=90,
        home=True,
    ...
    low = project_defcon_hit_probability(
        rows,
        position="DEF",
        xmins=45,
        home=True,
    ...
    assert high["projected_hit_probability"] > low["projected_hit_probability"]
    assert high["DEFCON_EV"] > low["DEFCON_EV"]


def test_stage_b_international_unavailable_unknown_is_not_injury_diagnosis():
    out = build_availability_state(
        _stage_b_player(),
        events=[
            {
                ...
                "claim": "Unavailable, reason not disclosed",
                "confidence": "MEDIUM",
            }
        ],
        snapshot_timestamp="2026-09-25T01:00:00Z",
        now_iso="2026-09-25T03:00:00Z",
    )
    assert "UNAVAILABLE_UNKNOWN" in out["states"]
    news = [row for row in out["evidence"] if row["source"] == "NEWS_REPORT"][0]
    assert news["medical_diagnosis"] is None
    assert out["governance"]["unknown_absence_not_relabelled_injury"] is True


def test_stage_b_played_90_international_minutes_is_structured_not_auto_penalty():
    out = build_availability_state(
        _stage_b_player(),
        events=[
            {
...
def test_stage_b_availability_conflict_preserves_official_authority():
    out = build_availability_state(
        _stage_b_player(),
        events=[
            {
                "source": "EXTERNAL_ANALYST",
                "timestamp": "2026-09-25T02:00:00Z",
                "state": "DOUBT",
                "confidence": "HIGH",
            }
    ...
    assert out["official_authority_present"] is True


def test_stage_b_stale_availability_cannot_be_dominant():
    out = build_availability_state(
        _stage_b_player(),
        events=[
            {
...
def test_stage_b_role_fact_is_not_overwritten_by_external_penalty_claim():
    player = _stage_b_player()
    rows = [
        _stage_b_dc_row(gw, starter=(gw != 5), minutes=90 if gw != 5 else 20)
        for gw in range(1, 6)
    ]
    out = build_role_duty_evidence(
        player,
        match_rows=rows,
        observed_role={
            "profile": "WIDE_ATTACKING_FULLBACK",
            "confidence": "MEDIUM",
            "source": "OBSERVED_MATCH_EVENTS",
        },
        external_claims=[
            {
                "name": "penalty_duty",
                ...
                "source": "EXTERNAL_ANALYST",
                "confidence": "HIGH",
                "claim": "Primary penalty taker",
            }
        ],
    )
    assert out["facts"]["penalty_duty"]["value"]["order"] == 2
    assert out["facts"]["penalty_duty"]["classification"] == "FACT"
    assert out["derived"]["actual_tactical_role"]["classification"] == "DERIVED"
    assert out["inferred"][0]["classification"] == "INFERRED"
    assert out["conflicts"][0]["resolution"] == "OFFICIAL_FACT_PRESERVED"


def test_stage_b_controlled_ab_reuses_p1_1_and_keeps_decisions_frozen():
    player = _stage_b_player(starts=4, minutes=360)
    rows = [
        _stage_b_dc_row(1, starter=True, minutes=90),
        _stage_b_dc_row(2, starter=False, minutes=20),
        _stage_b_dc_row(3, starter=False, minutes=15),
        _stage_b_dc_row(4, starter=False, minutes=10),
        _stage_b_dc_row(5, starter=False, minutes=12),
    ]
    role = build_role_duty_evidence(player, match_rows=rows)
    availability = build_availability_state(
        player,
        snapshot_timestamp="2026-09-25T01:00:00Z",
        now_iso="2026-09-25T03:00:00Z",
    ...
    out = run_stage_b_controlled_ab(
        ...
        candidate_defcon_ev=1.1,
        frozen_decision_surface={
            "player_ordering": [8101, 8102],
            "xi": [8101],
            "bench": [8102],
            "captain": 8101,
            "vice": 8102,
            "transfer_comparator": "HOLD",
        },
    )
    assert out["delta"]["xmins"] < 0
    assert out["delta"]["causal_fields"] == ["role_start_probability"]
    assert out["candidate_evidence"]["component_preview"]["DEFCON_EV"] == 1.1
    assert out["final_xpts_changed"] is False
    assert all(
        row["changed"] is False
        for row in out["decision_surface"].values()
    )


def test_stage_b_snapshot_is_deterministic():
    player = _stage_b_player()
    rows = [
        _stage_b_dc_row(gw, cbi=8 + gw % 2, tackles=3)
        for gw in range(1, 6)
    ]
    kwargs = {
        "bootstrap": {"elements": [player]},
        ...
        "now_iso": "2026-09-25T03:00:00Z",
        "snapshot_timestamp": "2026-09-25T02:00:00Z",
    }
    first = build_stage_b_evidence_snapshot(
        player_match_rows=rows,
        **kwargs,
    )
    second = build_stage_b_evidence_snapshot(
        player_match_rows=list(reversed(rows)),
        **kwargs,
    )
    assert json.dumps(first, sort_keys=True) == json.dumps(second, sort_keys=True)


def test_stage_b_feature_off_preserves_pre_stage_foundation_shape(
    tmp_path,
    monkeypatch,
):
...
def test_stage_b_feature_on_exposes_evidence_only_snapshot(
    tmp_path,
    monkeypatch,
):
    ...
    stage_b = out["stage_b_evidence"]
    assert stage_b["contract"] == "V12_STAGE_B_EVIDENCE_SNAPSHOT_V1"
    assert stage_b["governance"]["p1_7_semantics_changed"] is False
    assert stage_b["governance"]["p1_2b_semantics_changed"] is False
    assert stage_b["governance"]["monte_carlo_architecture_changed"] is False
