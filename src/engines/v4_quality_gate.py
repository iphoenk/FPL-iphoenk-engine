...
def _assert_orchestration(latest: dict) -> tuple[dict, list[dict]]:
    ...
    for key in (
        ...
        "personal_gw_scorecard_no_official_refetch", "finished_gw_archive_immutable",
        "scorecard_simulation_never_mutates_archive", "scorecard_projection_from_effective_plan_contract",
        "user_decision_overlay_process_isolated", "engine_is_advisory", "user_decision_is_final_authority",
        "engine_never_auto_overwrites_valid_user_override", "projection_default_baseline_previous_submitted_gw",
        "planning_override_requires_target_gw", "stale_planning_override_rejected", "optimizer_search_width_unchanged",
        "reconciliation_started_from_official_stats_starts", "minutes_never_infer_started", "missing_starts_excluded_from_brier",
        "effective_plan_legality_enforced_post_overlay", "engine_effective_plan_legality_reported_separately",
        "decision_compute_slo_excludes_external_network_io", "official_fpl_first_when_field_available",
        "dag_parallel_ready_services", "parallel_services_must_have_no_dependency_edge",
        "optimizer_may_parallelize_with_validation_before_preflight", "postflight_requires_preflight_and_effective_plan",
...
def _assert_prediction_and_validation(health: dict) -> tuple[dict, dict]:
    ...
    fixture_run_complete = sum((row.get("fixture_run") or {}).get("source") == "official_fpl_fixture_adjustment" and (row.get("fixture_run") or {}).get("decision_usage") == "multi_horizon_projection_context" for row in players)
...
def _assert_engine_advisory(latest: dict) -> tuple[dict, dict, dict, dict]:
    ...
    planning = pipeline.get("planning_squad") or {}
    assert planning.get("planning_gw") == (latest.get("projection_baseline") or {}).get("planning_gw")
    assert planning.get("baseline_gw") == (latest.get("projection_baseline") or {}).get("baseline_gw")
    assert planning.get("authority") == (latest.get("projection_baseline") or {}).get("effective_authority")
    assert planning.get("override_active") == (latest.get("projection_baseline") or {}).get("override_applied")
    return wc, packages, lineup, pipeline


def _assert_effective_plan(latest: dict) -> dict:
    plan = _load("effective_plan_v4.json")
    _assert_version(plan, "effective plan", 4941, "v4.9.4.1-user-decision-overlay")
    assert plan.get("status") == "PASS"
    assert plan.get("planning_gw") == (latest.get("projection_baseline") or {}).get("planning_gw")
    assert len((plan.get("effective_plan") or {}).get("starting_xi") or []) == 11
    assert (plan.get("effective_plan") or {}).get("formation") in LEGAL_FORMATIONS
    assert (plan.get("comparison") or {}).get("engine_can_warn_but_not_overwrite") is True
    ...
    assert guardrails.get("composition_from_effective_team_contract") is True
    return plan


def _assert_scorecard_and_report(latest: dict) -> tuple[dict, dict]:
    scorecard = _load("gw_scorecard_v4.json")
    checkpoint = _load("checkpoint_decision_v4.json")
    _assert_version(scorecard, "gw scorecard", 494, "v4.9.4-personal-gw-scorecard")
    assert scorecard.get("status") == "PASS"
    assert scorecard.get("team_id") == latest.get("team_id")
    ...
    assert (scorecard.get("guardrails") or {}).get("raw_snapshot_only") is True
    assert (scorecard.get("guardrails") or {}).get("official_api_refetch") is False
    assert (scorecard.get("guardrails") or {}).get("process_isolated_microservice") is True
    assert (scorecard.get("guardrails") or {}).get("finished_gw_archive_immutable") is True
    assert (scorecard.get("guardrails") or {}).get("simulation_never_mutates_archive") is True
    assert (scorecard.get("guardrails") or {}).get("projection_from_effective_plan_contract") is True
    assert (scorecard.get("guardrails") or {}).get("engine_is_advisory_user_plan_is_effective") is True
    assert (scorecard.get("guardrails") or {}).get("previous_submitted_gw_is_default_projection_baseline") is True
    assert (scorecard.get("guardrails") or {}).get("planning_override_target_gw_required") is True
    assert (scorecard.get("guardrails") or {}).get("projection_is_estimate_not_actual") is True
    assert (scorecard.get("guardrails") or {}).get("player_intervals_not_naively_summed") is True
...
def run() -> dict:
    ...
    # The top-level runtime artifact belongs to the canonical release, so gate it
    # against the release manifest instead of a stale component-era literal.
    _assert_version(latest, "latest", 496, f"{RELEASE_VERSION}-official-first-reporting", field="engine_version")
    ...
    wc, packages, lineup, pipeline = _assert_engine_advisory(latest)
    effective_plan = _assert_effective_plan(latest)
    scorecard, checkpoint = _assert_scorecard_and_report(latest)
    assert RELEASE_VERSION == "4.9.6"
    result = {
        "status": "PASS",
