...
from src.engines import framework_health_audit
...
def test_framework_lineup_probe_uses_canonical_derived_formation(tmp_path, monkeypatch):
    xi=[{"element":1,"position":"GK"}]+[{"element":i,"position":"DEF"} for i in range(2,5)]+[{"element":i,"position":"MID"} for i in range(5,10)]+[{"element":i,"position":"FWD"} for i in range(10,12)]
    (tmp_path / "lineup_decision_v4.json").write_text(json.dumps({
        "formation": "3-5-2",
        "starting_xi": xi,
        "guardrails": {"canonical_legality": True},
    }))
    monkeypatch.setattr(framework_health_audit, "DATA", tmp_path)
    ok, detail = framework_health_audit._probe_lineup()
    assert ok is True
    assert detail["derived_formation"] == "3-5-2"
