...
def _dss_traversal() -> dict[str, Any]:
    ...
    return {"core_ids":[str(x.get("id")) for x in core],"extension_ids":[str(x.get("id")) for x in ext],"core_count":len(core),"extension_count":len(ext),"all_modules_traversed":bool(core and ext)}
...
def _dimension_scores(player: dict[str, Any], cfg: dict[str, Any]) -> tuple[dict[str, float | None], dict[str, Any]]:
    ...
    start_p, exp_mins = _f(xmins.get("start_probability")), _f(xmins.get("expected_minutes"))
    x5, x15 = _f(player.get("xpts_5")), _f(player.get("xpts_15"))
    cost_m = max(.1, _f(player.get("now_cost"))/10.0)
    xg = _f(current.get("expected_goals") or current.get("xg") or hist.get("xg_per90")); xa = _f(current.get("expected_assists") or current.get("xa") or hist.get("xa_per90"))
    role_conf = _f(role.get("confidence") or role.get("role_confidence"), .6 if role else 0.0)
    set_piece = any(source.get(key) is not None for source in (role,hist,player) for key in ("penalty_share","penalty_role","set_piece_share","set_piece_role"))
    scores = {
    ...
    evidence = {"start_probability":start_p,"expected_minutes":exp_mins,"dnp_probability":_f(xmins.get("dnp_probability"),max(0.0,1-start_p)),"xpts_5":x5,"xpts_15":x15,"now_cost":player.get("now_cost"),"ownership_pct":player.get("ownership_pct"),"projection_confidence":player.get("projection_confidence"),"role_available":bool(role),"historical_prior_available":bool(hist),"set_piece_penalty_evidence":set_piece}
    return scores, evidence


def build_watchlist(prediction: dict[str, Any], team: dict[str, Any], dss: dict[str, Any] | None = None) -> dict[str, Any]:
    cfg = load_json_config(CONFIG); admission = cfg.get("admission") or {}; weights = {str(k):_f(v) for k,v in (cfg.get("dimension_weights") or {}).items()}
    positions = [str(x) for x in cfg.get("positions") or ["GK","DEF","MID","FWD"]]; owned = _owned_ids(team); ranked = {p:[] for p in positions}; rejected=0
    traversal = _dss_traversal(); dss = dss or {}; critical_dss_failed = bool((dss.get("summary") or {}).get("critical_failed", False))
    for player in prediction.get("players") or []:
        ...
        scores,evidence=_dimension_scores(player,cfg); available=sum(weights.get(k,0) for k,v in scores.items() if v is not None); total=sum(weights.values()) or 1; coverage=available/total
        critical=[scores.get(str(k)) for k in admission.get("critical_dimensions") or []]; critical_available=[_f(x) for x in critical if x is not None]; critical_score=min(critical_available) if critical_available else 0.0
    ...
    return {"schema_version":int(cfg.get("schema_version") or 1),"model":cfg.get("model_id"),"screening_contract":cfg.get("screening_contract"),"status":"READY" if count else "INSUFFICIENT_EVIDENCE","positions":ranked,"candidate_count":count,"rejected":rejected,"owned_excluded":len(owned),"dss_traversal":traversal,"governance":cfg.get("governance") or {}}
