...
def test_compact_package_audit_matches_reference_acceptance_snapshot():
    ...
    assert perf["stable_top_package_ties"] is True
    assert perf["target_metric_cache_removed"] is True
    assert perf["full_package_dicts_avoided"] > 0
    assert perf["metrics_cache_kind"] == "outgoing_keep_profiles_reused_across_incoming_states"
