...
# Official element_type mapping: 1 GK, 2 DEF, 3 MID, 4 FWD.
...
# We persist only the 2026/27 deltas that are explicitly published and useful to our model.
# Full BPS remains Official-FPL/Opta-derived and must not be reverse-engineered when raw metrics are unavailable.
BPS_2026_27 = {
...
def score_from_official_stats(stats: dict, element_type: int) -> dict:
    """Reconstruct published FPL match points from Official event-live style stats.

    This is an audit helper, not a replacement for Official total_points. Clean-sheet and
    goals-conceded values are assumed to be player-specific Official stats. If DC is missing,
    the audit is marked incomplete rather than silently assuming zero.
    """
    ...
    if minutes >= 60 and int(stats.get("clean_sheets") or 0) > 0:
    ...
    if pos in (1, 2):
        components["goals_conceded"] = (int(stats.get("goals_conceded") or 0) // GOALS_CONCEDED_INTERVAL) * GOALS_CONCEDED_POINTS_PER_INTERVAL
    dc_present = "defensive_contribution" in stats
    if dc_present:
        components["defensive_contributions"] = defensive_contribution_points(pos, stats.get("defensive_contribution") or 0)
    total = sum(components.values())
    return {"points": total, "components": components, "complete": dc_present or pos == 1, "ruleset_id": RULESET_ID}
...
def build_chip_ledger(used_chips: list[dict] | None, current_gw: int | None = None) -> dict:
    ...
    next_half = chip_half(current_gw or 1)
    return {
