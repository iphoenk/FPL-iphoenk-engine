...
def _role_from_projection(player: dict[str, Any]) -> dict[str, Any]:
    ...
    if not role:return {}
    return {"role":role.get("role") or role.get("tactical_role"),"set_piece_share":role.get("set_piece_share"),"penalty_share":role.get("penalty_share"),"return_routes":role.get("return_routes") or [],"progression_route":role.get("progression_route"),"source":role.get("set_piece_source") or "v5_role_intelligence"}
...
def attach_tactical_matchups(predictions: dict[str, Any], planning_gw: int, tactical_context: dict[str, Any] | None = None) -> dict[str, Any]:
    cfg=load_json_config(CONFIG); context=tactical_context if isinstance(tactical_context,dict) else {}; teams=_by_id(context.get("team_profiles"),"teams"); explicit_roles=_by_id(context.get("player_roles"),"players"); recent=_recent(context.get("recent_form")); minimum=int((cfg.get("materiality") or {}).get("minimum_evidence_items") or 2); window=int(cfg.get("recent_gw_window") or 5); limit=int((cfg.get("materiality") or {}).get("maximum_report_highlights_per_player") or 2); ready=partial=unavailable=0
    for player in predictions.get("players") or []:
        ...
        own=teams.get(team_id) or {}; opponent=teams.get(opponent_id) or {}; role=explicit_roles.get(element) or _role_from_projection(player); recent_rows=sorted(recent.get(opponent_id) or [],key=lambda x:int(x.get("gw") or 0),reverse=True)[:window]; tactical_evidence=sum(bool(x) for x in (own,opponent,recent_rows)); evidence_count=tactical_evidence+int(bool(role))
        ...
        player["tactical_matchup"]={"status":status,"planning_gw":planning_gw,"opponent_team_id":opponent_id if opponent_id>0 else None,"coach":own.get("coach"),"own_shape":own.get("base_formation"),"opponent_coach":opponent.get("coach"),"opponent_shape":opponent.get("base_formation"),"opponent_shape_evidence":opponent.get("evidence_class"),"player_role":role.get("role"),"player_return_routes":list(role.get("return_routes") or []),"player_progression_route":role.get("progression_route"),"opponent_strengths":list(opponent.get("strengths") or []),"opponent_vulnerabilities":list(opponent.get("vulnerabilities") or []),"opponent_observed_style_proxies":list(opponent.get("observed_style_proxies") or []),"evidence_confidence":opponent.get("confidence") or (recent_rows[0].get("confidence") if recent_rows else None),"evidence_count":evidence_count,"recent_gw_evidence_count":len(recent_rows),"highlights":_highlights(opponent,role,recent_rows,limit) if tactical_evidence else [],"advisory_only":True,"xpts_mutated":False}
    predictions["tactical_matchup_summary"]={"model":cfg.get("model_id"),"planning_gw":planning_gw,"ready":ready,"partial":partial,"unavailable":unavailable,"context_supplied":bool(context),"advisory_only":True,"xpts_mutation":False}
    predictions.setdefault("governance",{})["tactical_matchup"]={"background_analysis_required_for_owned_and_watchlist":True,"report_only_material_highlights":True,"never_directly_mutate_xpts":True,"allow_selection_tiebreaker_only_when_gap_is_close":True,"missing_evidence_is_never_fabricated":True}
    return predictions
