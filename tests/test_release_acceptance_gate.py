...
def test_composite_release_gate_preserves_all_underlying_integrations():
    ...
    assert any("incremental_domain_orchestrator --mode daily --stats --profile full_refresh" in command for command in commands)
    ...
    fast_commands = [command for command in commands if "incremental_domain_orchestrator --mode daily --stats --profile fast_decision" in command]
