...
def _manifest_satisfies_checkpoint(
    manifest: dict[str, Any],
    *,
    target_utc: datetime,
    source_commit: str,
) -> bool:
    """Return true when the current logical checkpoint already has valid work.

    An exact targeted precompute satisfies the checkpoint before :30. After the
    target, any complete same-source snapshot generated at or after the logical
    checkpoint also satisfies it. This lets workflow-run/manual refreshes count
    as genuine recovery evidence without weakening targeted-precompute rules.
    """
    target_utc = target_utc.astimezone(timezone.utc)
    checkpoint = manifest.get("checkpoint") or {}
    if checkpoint.get("materialization_complete") is not True:
...
def _adaptive_recovery_target(now_utc: datetime) -> tuple[str | None, datetime | None]:
    """Classify an adaptive wake-up without changing logical checkpoint owners."""
    local = now_utc.astimezone(timezone.utc).astimezone(_local_tz())
    target_minute = int(_policy().get("visible_checkpoint_minute") or 30)
...
def _checkpoint_recovery_decision(now_utc: datetime, target: datetime) -> dict[str, Any]:
    """Recover one missed logical :30 checkpoint using the existing authority."""
