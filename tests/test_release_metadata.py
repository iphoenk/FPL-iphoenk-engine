...
def test_release_metadata_surfaces_are_consistent():
    ...
    assert service_registry["schema_version"] == 12
    assert service_registry["production_contract"].startswith("v3.20.2-")
    assert source_registry["registry"] == "SOURCE_REGISTRY_V3"
    assert runtime_artifact_registry["registry"] == "RUNTIME_ARTIFACT_CONTRACTS_V1"
