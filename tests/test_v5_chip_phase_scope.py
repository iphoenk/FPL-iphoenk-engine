...
def test_legacy_wildcard_flag_does_not_leak_into_next_planning_gw():
    ...
    assert state["user_lock_chip_scope"] == "STALE_PREVIOUS_GW_IGNORED"
    assert state["governance"]["chip_activation_is_gameweek_scoped"] is True


def test_legacy_wildcard_flag_still_applies_within_same_planning_gw():
    ...
    assert state["source"] == "user_lock"
    assert state["user_lock_chip_applies_to_planning_gw"] is True
    assert state["user_lock_chip_scope"] == "LEGACY_LOCK_CURRENT_GW"


def test_explicit_lock_gw_controls_chip_scope_independently_of_squad_persistence():
    applies, reason = _lock_chip_scope({"wildcard_active": True, "planning_gw": 3}, planning_gw=3, submitted_gw=2)
    stale, stale_reason = _lock_chip_scope({"wildcard_active": True, "planning_gw": 2}, planning_gw=3, submitted_gw=2)

    assert applies is True
    assert reason == "EXPLICIT_GW_MATCH"
