...
def _gw_row(player: dict[str, Any], gw: int) -> dict[str, Any]:
    return next((row for row in player.get("xpts_by_gw") or [] if int(row.get("gw") or -1) == int(gw)), {"mean": 0.0, "std": 0.0})
...
def best_lineup(players: list[dict[str, Any]], gw: int, lineup_rules: dict[str, Any]) -> dict[str, Any]:
    ...
    for formation in lineup_rules.get("legal_formations") or []:
        ...
        for pos, count in (("DEF", d), ("MID", m), ("FWD", f)):
            selected.extend(sorted([p for p in players if p.get("position") == pos], key=lambda p: _f(_gw_row(p, gw).get("mean")), reverse=True)[:count])
        if len(selected) != int(lineup_rules.get("starting_xi_size") or 11):
            continue
...
def score_package(players: list[dict[str, Any]], planning_gw: int, rules: dict[str, Any], changes: int = 0) -> dict[str, Any]:
    ...
    for horizon in horizons:
        ...
        for offset in range(horizon):
            ...
            captain = max(((_f(_gw_row(p, gw).get("mean")), _f(_gw_row(p, gw).get("std"))) for p in players if int(p["element"]) in starter_ids), default=(0.0, 0.0))
        ...
        results[str(horizon)] = {"valid": valid, "mean": round(total_mean, 3) if valid else None, "std": round(math.sqrt(total_var), 3) if valid else None}
    ...
    return {"valid": True, "horizons": results, "objective_mean": round(mean, 3), "objective_std": round(std, 3), "robust_score": round(robust, 3)}
...
def build_packages(prediction: dict[str, Any], team: dict[str, Any], rules: dict[str, Any]) -> dict[str, Any]:
    ...
    if not finance.get("sell_value_complete") or not legal_squad(current, rules.get("squad") or {}):
        return {"status": "BLOCKED", "reason": "finance incomplete or current squad illegal", "packages": [], "gate0_prevalidated": False}
    ...
    for position in ("GK", "DEF", "MID", "FWD"):
        rows = []
        for p in players:
            if p.get("position") != position or int(p["element"]) in owned_ids or p.get("status") not in allowed:
                continue
            candidate = {**p, "sell_cost": p.get("now_cost")}
            h5 = (p.get("horizons") or {}).get("5") or {}
    ...
    packages = [{"id": "HOLD", "changes": 0, "outs": [], "ins": [], "affordability": {"resulting_itb": itb}, "score": hold_score, "legal": True}]
    singles = []
    for out in current:
        for incoming in pools.get(out["position"], [])[: int(cfg.get("max_single_moves_per_out") or 4)]:
            ok, money = affordable([out], [incoming], itb)
            candidate = [p for p in current if int(p["element"]) != int(out["element"])] + [incoming]
            if not ok or not legal_squad(candidate, rules.get("squad") or {}):
                continue
            score = score_package(candidate, planning_gw, rules, changes=1)
            singles.append((out, incoming, candidate, money, score))
            packages.append({"id": f"1:{out['element']}->{incoming['element']}", "changes": 1, "outs": [{"element": out["element"], "name": out.get("name"), "sell_cost": out.get("sell_cost")}], "ins": [{"element": incoming["element"], "name": incoming.get("name"), "now_cost": incoming.get("now_cost")}], "affordability": money, "score": score, "legal": True})
    ...
    if int(cfg.get("max_changes") or 2) >= 2:
        for i, a in enumerate(seeds):
            for b in seeds[i + 1:]:
                ...
                if not ok or not legal_squad(candidate, rules.get("squad") or {}): continue
                score = score_package(candidate, planning_gw, rules, changes=2)
                packages.append({"id": f"2:{outs[0]['element']},{outs[1]['element']}->{ins[0]['element']},{ins[1]['element']}", "changes": 2, "outs": [{"element": x["element"], "name": x.get("name"), "sell_cost": x.get("sell_cost")} for x in outs], "ins": [{"element": x["element"], "name": x.get("name"), "now_cost": x.get("now_cost")} for x in ins], "affordability": money, "score": score, "legal": True})
                if len(packages) >= int(cfg.get("max_deterministic_packages") or 220): break
            if len(packages) >= int(cfg.get("max_deterministic_packages") or 220): break
    ...
    for idx, package in enumerate(packages[:top_n]):
        ...
        mc = _simulate(mean, std, int(cfg.get("monte_carlo_simulations") or 300), int(cfg.get("monte_carlo_seed") or 1) + idx)
        diff_std = math.sqrt(std ** 2 + hold_std ** 2)
        mc["p_outperform_hold_independent_baseline"] = round(1.0 - NormalDist(mu=mean - hold_mean, sigma=diff_std).cdf(0.0), 4) if diff_std > 0 else (1.0 if mean > hold_mean else 0.5)
        package["monte_carlo"] = mc
    return {
        "model": cfg.get("model_id"), "status": "READY", "planning_gw": planning_gw, "ruleset_id": rules.get("ruleset_id"), "gate0_prevalidated": True,
        "simulation_assumption": cfg.get("simulation_assumption"), "package_count": len(packages), "hold": next((p for p in packages if p["id"] == "HOLD"), None),
        "candidate_pool": {pos: [{"element": p["element"], "name": p.get("name"), "now_cost": p.get("now_cost"), "candidate_score": round(_f(p.get("candidate_score")), 3)} for p in rows] for pos, rows in pools.items()},
        "packages": packages[:top_n], "governance": {"candidate_generation_only": True, "final_go_requires_framework_governance_and_postflight_gate0": True, "price_uses_sell_value_for_outs_and_now_cost_for_ins": True},
    }
