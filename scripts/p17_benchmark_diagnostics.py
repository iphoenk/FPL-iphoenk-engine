...
def _build_squads(route_count: int, *, tie_rich: bool = False):
...
def _run_batch(route_count: int, *, tie_rich: bool = False) -> dict[str, Any]:
    fixture, projections, squads = _build_squads(route_count, tie_rich=tie_rich)
    ...
    return {
        "mode": "batch",
        "fixture": "tie_rich" if tie_rich else "normal",
...
def _child(mode: str, route_count: int, output: Path, pstats_path: Path | None, tie_rich: bool = False):
    if mode == "batch":
        payload = _run_batch(route_count, tie_rich=tie_rich)
    elif mode == "scalar":
...
def _render_md(payload: dict[str, Any]) -> str:
    ...
    tie_repeated = payload["batch_2043_tie_rich_repeated"]
    lines = [
    ...
    lines.extend(
        [
            ...
            "",
            "## Five independent tie-rich batch processes",
            "",
            "| Run | Seconds |",
            "|---:|---:|",
        ]
    ...
    for index, seconds in enumerate(
        tie_repeated["samples_seconds"],
        start=1,
    ):
        lines.append(f"| {index} | {seconds:.6f} |")
    tie_s = tie_repeated["summary"]
    lines.extend(
        [
            "",
            f"- min: {tie_s['min']:.6f} s",
            f"- median: {tie_s['median']:.6f} s",
            f"- max: {tie_s['max']:.6f} s",
            f"- mean: {tie_s['mean']:.6f} s",
            f"- range: {tie_s['range']:.6f} s",
            "",
...
def main() -> int:
    ...
    parser.add_argument("--tie-rich", action="store_true")
    ...
    for index in range(args.repeats):
        row = _invoke_child(
            mode="batch",
            route_count=2043,
            output=artifact_dir / f"batch_2043_tie_rich_run_{index + 1}.json",
            tie_rich=True,
        )
        tie_samples.append(float(row["elapsed_seconds"]))
        tie_proofs.append(dict(row.get("proof") or {}))
    ...
    payload = {
        ...
        "batch_2043_tie_rich_repeated": {
            "process_isolation": True,
            "samples_seconds": tie_samples,
            "summary": _summary(tie_samples),
            "proofs": tie_proofs,
        },
