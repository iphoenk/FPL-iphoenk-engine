...
def test_prediction_runtime_identity_matches_canonical_release() -> None:
    ...
    quality_gate_core = (root / "src" / "engines" / "v4_quality_gate_core.py").read_text()
    ...
    assert "from src.release import RELEASE_VERSION" in quality_gate_core
    assert '_assert_version(obj, phase, 492, f"v{RELEASE_VERSION}-truthful-health")' in quality_gate_core
    assert "from src.release import RELEASE_VERSION" in quality_gate_legacy
    assert "RELEASE_VERSION = core.RELEASE_VERSION" not in quality_gate_legacy
