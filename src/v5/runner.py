...
def _request_specs(section: str, tokens: dict[str, Any]) -> dict[str, FetchSpec]:
    ...
    for name, raw in raw_section.items():
        ...
        if when and not tokens.get(str(when)):
            continue
        params = {str(key): _resolve_token(value, tokens) for key, value in (raw.get("params") or {}).items()}
...
def _scope_auth_summary(runtime: dict[str, Any], owned_ids: list[int]) -> dict[str, Any]:
    ...
    draft = {int(row["element"]) for row in (my_team or {}).get("picks", []) or [] if row.get("element") is not None}
    ...
    summary["role"] = "OPTIONAL_PRIVATE_ENRICHMENT"
    summary["production_blocking"] = False
    return summary


def assemble_snapshot(
    *, mode: str, base_payloads: dict[str, Any], base_health: dict[str, dict] | None = None,
    dynamic_payloads: dict[str, Any] | None = None, dynamic_health: dict[str, dict] | None = None,
    auth_runtime: dict[str, Any] | None = None, locked_squad: dict | None = None,
    previous_price_state: dict | None = None, predictions: dict | None = None, now: datetime | None = None,
) -> dict[str, Any]:
    ...
    live = personalized_live_score(
        ...
        identity=identity, scoring_gw=context.scoring_gw, is_live_event=context.is_live_event,
    )
    price_bundle = build_price_snapshot(bootstrap, previous_state=previous_price_state or {}, owned_ids=owned_ids, now=current_time)
    prediction_payload = predictions or {"status": "NOT_BUILT", "players": [], "v5_bridge": {"enabled": False}}
    ...
    snapshot = {
        ...
        "authority_policy": team.get("authority_policy", {}),
        "team_summary": {
            "bank": team["finance"].get("bank"), "market_value": team["finance"].get("market_value"),
            "sell_value": team["finance"].get("sell_value"), "sell_value_complete": team["finance"].get("sell_value_complete"),
            "finance_exact_count": team["finance"].get("exact_count"), "unresolved_sell_values": team["finance"].get("unresolved_elements"),
        },
        "live_summary": {"status": live.get("status"), "gross_points": live.get("gross_points"), "net_points": live.get("net_points")},
        "price_summary": {
        ...
        "prediction_summary": {
            "status": prediction_payload.get("status", "BUILT"), "model_version": prediction_payload.get("model_version"),
            "player_count": len(prediction_payload.get("players", []) or []), "v5_bridge": prediction_payload.get("v5_bridge", {}),
        },
        ...
        "governance": {
            ...
            "pipeline": list(_cfg()["pipeline"]), "raw_authenticated_payload_persisted": False,
            "authority_model": "PUBLIC_OFFICIAL_PLUS_USER_CAPTURE",
            "authenticated_official_production_blocking": False,
        },
    }
    return {"snapshot": snapshot, "team": team, "live": live, "prices": price_bundle["prices"], "price_trajectory": price_bundle["trajectory_state"], "price_alerts": price_bundle["alerts"], "predictions": prediction_payload, "context": context}


def run(mode: str | None = None, *, persist: bool = True, include_predictions: bool = True) -> dict[str, Any]:
    timer = PipelineTimer(); cfg = _cfg(); selected_mode = mode or str(cfg["default_mode"])
    if selected_mode not in set(str(x) for x in cfg["modes"]):
        raise ValueError(f"unsupported V5 runner mode: {selected_mode}")
    team_id = expected_team_id(); tokens = {"team_id": team_id}
    with timer.stage("public_base_collection"):
        base_payloads, base_health = fetch_many(_request_specs("base_requests", tokens))
    bootstrap = base_payloads.get("bootstrap")
    if not isinstance(bootstrap, dict): raise RuntimeError("V5 FAIL CLOSED: bootstrap unavailable")
    with timer.stage("event_context"): context = build_event_context(bootstrap)
    tokens.update({"submitted_gw": context.submitted_gw, "scoring_gw": context.scoring_gw, "planning_gw": context.planning_gw})
    ...
    with timer.stage("dynamic_and_authenticated_collection"):
        ...
        if cfg["concurrency"].get("parallelize_dynamic_and_authenticated_collection", True):
            with ThreadPoolExecutor(max_workers=max(1, workers)) as pool:
                ...
                auth_future = pool.submit(collect_runtime, ()) if cfg.get("feature_switches", {}).get("authenticated_overlay", True) else None
            ...
            auth_runtime = collect_runtime(()) if cfg.get("feature_switches", {}).get("authenticated_overlay", True) else {"summary": {"state": "DISABLED"}, "my_team": None}
    ...
    if include_predictions and cfg.get("feature_switches", {}).get("v4_prediction_bridge", True):
        with timer.stage("prediction_bridge"):
            ...
            predictions = build_predictions(bootstrap, base_payloads.get("fixtures") if isinstance(base_payloads.get("fixtures"), list) else [], iso_now(), stats_gw=stats_gw)
    with timer.stage("assemble_runtime"):
        result = assemble_snapshot(mode=selected_mode, base_payloads=base_payloads, base_health=base_health, dynamic_payloads=dynamic_payloads, dynamic_health=dynamic_health, auth_runtime=auth_runtime, locked_squad=lock, previous_price_state=previous_price_state, predictions=predictions)
    ...
    if persist and cfg.get("feature_switches", {}).get("persistence", True):
        with timer.stage("persistence"):
            write_artifact("health", {"generated_at": result["snapshot"]["generated_at"], "public": result["snapshot"]["endpoint_health"], "authenticated": result["snapshot"]["authenticated_official"].get("endpoint_health", {})})
            write_artifact("team", result["team"]); write_artifact("live", result["live"]); write_artifact("prices", result["prices"]); write_artifact("price_trajectory", result["price_trajectory"]); write_artifact("price_alerts", result["price_alerts"]); write_artifact("predictions", result["predictions"])
        result["snapshot"]["performance"] = timer.report(); gw = result["context"].submitted_gw or result["context"].planning_gw; result["snapshot"]["files"] = write_snapshot(result["snapshot"], gw=gw)
    return result["snapshot"]


def cli() -> None:
    cfg = _cfg(); parser = argparse.ArgumentParser(description="FPL iphoenk Engine V5 alpha runner"); parser.add_argument("mode", choices=tuple(str(x) for x in cfg["modes"]), nargs="?", default=str(cfg["default_mode"])); parser.add_argument("--no-persist", action="store_true"); parser.add_argument("--no-predictions", action="store_true"); args = parser.parse_args(); print(json.dumps(run(args.mode, persist=not args.no_persist, include_predictions=not args.no_predictions), ensure_ascii=False, indent=2))
