        ...
        ledger.append(
            {
                **row,
                ...
                "purchase_cost": 50,
                "sell_cost": 50,
                "finance_source": "synthetic_exact",
                "finance_exact": True,
            }
        )
    return {
...
def _prediction() -> dict:
    ...
    for element, position in enumerate(_positions(), start=1):
        ...
        horizons = {
            str(h): {
                "mean": round(sum(row["mean"] for row in by_gw[:h]), 3),
                "std": round((h * 1.2**2) ** 0.5, 3),
            }
            for h in (3, 5, 10, 15)
        }
        players.append(
...
def _price() -> dict:
    return {
        "prices": {},
        "alerts": {"alerts": []},
        "capabilities": ["transfer_momentum", "price_intelligence"],
    }


def _evaluation() -> dict:
    return {
        "capabilities": ["prediction_evaluation", "calibration_store", "challenger_scorecard"],
        "accuracy": {"overall": {"sample_size": 0}},
    }
...
def test_native_lineup_is_legal_and_governed():
    lineup = optimize_lineup(_truth()["team"], _prediction(), _truth()["rules"])
...
def test_dss_uses_capability_contract_and_does_not_fake_missing_modules():
    ...
    assert core["manager_tactical_role"]["status"] == "PARTIAL"
