...
def _metrics(forecasts: list[dict[str, Any]], actuals: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "starter_brier": round(
            _brier([_f(f["start_probability"]) for f, _ in starters], [_f(a["started"]) for _, a in starters]) or 0.0,
            4,
        ) if starters else None,
        "starter_sample_size": len(starters),
        "clean_sheet_brier": round(
            _brier([_f(f["clean_sheet_probability"]) for f, _ in cs], [_f(a["clean_sheet"]) for _, a in cs]) or 0.0,
            4,
        ) if cs else None,
...
def _historical_event_live(event_live_by_gw: dict[str, Any] | None, gw: int) -> dict[str, Any] | None:
    if not isinstance(event_live_by_gw, dict):
        return None
    value = event_live_by_gw.get(str(gw))
    if value is None:
        value = event_live_by_gw.get(gw)  # type: ignore[index]
...
def evaluate(
    ...
    *,
    ...
    settlement_health: dict[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    for key, record in list(records.items()):
        ...
        if event_live and gw == live_gw:
            actual_payload = event_live
            actual_source = "CURRENT_SCORING_EVENT_LIVE"
        else:
            historical = _historical_event_live(event_live_by_gw, gw)
            if historical is not None:
                actual_payload = historical
                actual_source = "HISTORICAL_FINISHED_EVENT_LIVE"
        if actual_payload is None or actual_source is None:
            continue
        ...
        record["settlement_provenance"] = {
            "source": actual_source,
            "official_event_live": True,
            "event_finished": True,
            "postdeadline_prediction_reconstruction": False,
            "frozen_forecast_mutated": False,
        }
        records[key] = record
        settled_sources[str(gw)] = actual_source
    ...
    confidence = "LOW" if sample <= int(thresholds.get("low_max") or 49) else (
        "MEDIUM" if sample <= int(thresholds.get("medium_max") or 149) else "HIGH"
    )
    ...
    health = settlement_health if isinstance(settlement_health, dict) else {}
    historical_payload_count = len(event_live_by_gw or {}) if isinstance(event_live_by_gw, dict) else 0
    settlement_source_health = {
        "current_event_live_available": bool(event_live),
        "historical_event_live_requested": len(health.get("requested_gameweeks") or []),
        "historical_event_live_fetched": historical_payload_count,
        "historical_event_live_health": health.get("health_by_gw") if isinstance(health.get("health_by_gw"), dict) else {},
        "settled_sources": settled_sources,
        "missing_historical_actuals_fail_neutral": True,
    }
    accuracy = {
        "generated_at": _now(),
        ...
        "governance": {
            ...
            "historical_event_live_supplies_actuals_only": True,
...
def challenger_scorecard(prediction: dict[str, Any], observations: dict[str, Any] | None, accuracy: dict[str, Any]) -> dict[str, Any]:
        ...
        providers.append(
            {
                ...
                "historical_sample": len([r for r in rows if str(r.get("provider")) == pid and r.get("actual") is not None]),
    ...
    return {
        ...
        "status": "ACTIVE" if any(p["state"] == "OBSERVED" for p in providers if p["id"] != "internal") else "ACTIVE_INTERNAL_ONLY_EXTERNAL_DATA_ABSENT",
    }
