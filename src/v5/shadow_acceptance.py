...
def _runtime_reference_matches(context:dict[str,Any],v3:dict[str,Any])->bool:
    ...
    if str(v3.get("engine_version") or "")!=expected:return False
...
def validated_cycle_eligible(payload:dict[str,Any])->bool:
    ...
    if policy.get("require_same_production_baseline",True) and (str(context.get("production_baseline_version") or "")!=baseline_version or str(context.get("production_main_sha") or "")!=baseline_sha or not _runtime_reference_matches(context,v3)):return False
...
def finalize(latest_path:str,output_dir:str)->dict[str,Any]:
    ...
    baseline_version,baseline_sha=_baseline(); context=payload.get("acceptance_context") if isinstance(payload.get("acceptance_context"),dict) else {}; v3=payload.get("v3") if isinstance(payload.get("v3"),dict) else {}; current_fingerprint=_current_release_fingerprint()
    ...
    if not _runtime_reference_matches(context,v3):raise RuntimeError("shadow cycle V3 runtime implementation does not match its declared runtime reference")
