...
def test_precompute_targets_same_hour_30_when_started_before_45_wib() -> None:
    # 21:15 UTC == 04:15 WIB on the following local day.
...
def test_primary_heartbeat_resolves_most_recent_logical_30() -> None:
    assert pc.current_logical_checkpoint(datetime(2026, 8, 30, 21, 31, tzinfo=timezone.utc)) == datetime(
        2026, 8, 30, 21, 30, tzinfo=timezone.utc
    )
    assert pc.current_logical_checkpoint(datetime(2026, 8, 30, 22, 5, tzinfo=timezone.utc)) == datetime(
        2026, 8, 30, 21, 30, tzinfo=timezone.utc
    )
...
def test_primary_skips_only_exact_on_time_same_commit_precompute() -> None:
    ...
    wrong_target = {
        **manifest,
        "checkpoint": {**manifest["checkpoint"], "target_checkpoint": "2026-08-30T22:30:00+00:00"},
    }
