...
def test_current_metadata_uses_canonical_fast_and_topology_contracts() -> None:
    ...
    assert current["topology_authority"] == "config/runtime/execution_domains.json + config/v3_service_registry.json"
    assert "HISTORICAL_REFERENCE_NOT_CURRENT_AUTHORITY" in current["runtime_metrics_semantics"]
...
def test_historical_evidence_is_not_presented_as_current_runtime_authority() -> None:
    ...
    assert status["release_closeout"]["topology_semantics"] == "HISTORICAL_AT_TIME"
    assert status["architecture_closeout"]["topology_semantics"] == "HISTORICAL_AT_TIME"
    assert status["release_closeout"]["execution_domain_count"] == 11
    assert status["architecture_closeout"]["execution_domain_count"] == 11
