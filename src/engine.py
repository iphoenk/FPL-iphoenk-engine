...
def run(mode="daily",sync_stats=False,deep_stats=False):
 ...
 if sync_stats and stats_gw:
  ci=core_insights.sync_gw(stats_gw); vv=vaastav.sync_gw(stats_gw); pv=vaastav.sync_previous_season(); adv_summary={"core_insights":{"ok":bool(ci.get("schema_valid")),"rows":ci.get("row_count"),"error":ci.get("error")},"vaastav":{"ok":bool(vv.get("rows")),"rows":vv.get("row_count"),"error":vv.get("error")},"last_season":{"ok":bool(pv.get("rows")),"season":pv.get("season"),"rows":pv.get("row_count"),"error":pv.get("error")}}
