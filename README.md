...
> **Last runtime/documentation sync:** `2026-09-23T17:10:00+07:00`  
...
Full-direct P1.2B now has an execution-only **route-batch P1.7 kernel** for large route sets. Player surfaces and legal-XI templates are shared, all 550 legal XI and all six bench permutations remain numerically evaluated for every direct squad, and canonical C/VC ordering is preserved. Starter aggregates use eleven ordered binary64 vector additions in the same legal-XI index order as scalar P1.7, then the exact canonical route sort key is resolved vectorially with stable tie semantics. Cameo-blocking counterfactual probability mass is evaluated in the same route/bench tensor and only the selected winner's exact value is carried into package utility; publish-only alternative detail is not recomputed. Batch evidence fingerprints are derived from the exact player surfaces plus deterministic selected output rather than repeatedly serializing the full raw projection universe. Package routes are never pruned; small/funded route sets keep the scalar canonical path, and the scalar owner remains the CI correctness oracle. The full-direct CI acceptance remains hard-gated at 2,043 exact route squads in at most 10 seconds.
