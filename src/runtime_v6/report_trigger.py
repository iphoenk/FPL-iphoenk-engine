...
def _encoded_report_type(
    *,
    request_id: str,
    requested_at: datetime,
    report_type: str,
) -> str:
    return f"ADHOC:{request_id}:{_request_time_token(requested_at)}:{report_type}"
...
def build_ad_hoc_report_context(
    *,
    ...
    Identity is request_id + the original timezone-normalized request timestamp
    + report type. The report-slot timestamp remains minute-aligned for the
    existing delivery contract; request seconds (and microseconds when present)
    are encoded in the slot type so two requests cannot collide merely because
...
def resolve_ad_hoc_report_decision(
    *,
    ...
    canonical_requested_at = _parse_aware_timestamp(
